...
private:
	struct BuildDesc
	{
		...
		// 오디오 믹싱 버스(이름 + 기본 음량) — 패키지 게임이 ConfigureBuses 로 주입받는다.
	...
	bool BuildAndroidPackage(const BuildDesc& desc, std::string& outError) const;
	...
	bool VerifyAndroidPackage(const BuildDesc& desc, std::string& outError) const;
