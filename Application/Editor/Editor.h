...
public:
	...
	inline static SafePtr<CCpuProfilerWindow>		CpuProfiler = nullptr;
	...
	// 캔버스 선택(오브젝트 + 레이어 + 캔버스) 해제. 레이어/캔버스까지 지우는 이유: 호출부(캔버스뷰
	// 빈 곳 클릭, 에셋 선택, 활성 캔버스 없음)는 전부 "캔버스에서 고른 것을 없던 일로" 를 뜻한다.
	...
	// ── 캔버스 선택 ───────────────────────────────────────────────────────────
	// 캔버스는 런타임에 하나(활성 캔버스)뿐이라 대상 포인터가 필요 없다 — 켜짐/꺼짐만 든다.
	...
	// 레이어는 캔버스 소유(OwnerPtr)이므로 SafePtr 로 들고, 파괴되면 자동 무효화된다.
