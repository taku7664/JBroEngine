...
namespace EditorGuiActions
{
	...
	bool DrawAddComponentMenu(CGameCanvas& canvas, CGameObject* object);

	// 버튼 + 필터 가능한 팝업 조합으로 컴포넌트 추가 UI 표시.
	bool DrawAddComponentButton(CGameCanvas& canvas, CGameObject* object, const char* buttonLabel = nullptr);
	...
	bool DrawAddObjectMenu(CGameCanvas& canvas, CGameObject* parent, const Vector2* spawnWorldPos = nullptr,
	                       CGameLayer* layer = nullptr);

	bool DrawRemoveObjectMenu(CGameCanvas& canvas, CGameObject* object);
	...
	// 클립보드의 오브젝트를 canvas 에 붙여넣는 MenuItem(클립보드가 오브젝트일 때만 표시).
	//   layer == nullptr 이면 현재 선택으로 대상 레이어를 정한다(ResolveTargetLayer).
	bool DrawPasteObjectMenuItem(CGameCanvas& canvas, CGameObject* parent = nullptr,
	                             CGameLayer* layer = nullptr);
	...
	CGameLayer* ResolveTargetLayer(CGameCanvas& canvas);
	...
	bool PasteObjectsFromClipboard(CGameCanvas& canvas, const Vector2* spawnWorldPos = nullptr,
	                               CGameObject* parent = nullptr, CGameLayer* layer = nullptr);
	// 선택된 최상위 오브젝트들을 한 번의 undo 단위로 삭제한다.
	bool DeleteSelectedObjects(CGameCanvas& canvas);
	// 선택된 레이어를 삭제한다(마지막 레이어면 거부). 성공 시 레이어 선택 해제.
	bool DeleteSelectedLayer(CGameCanvas& canvas);
