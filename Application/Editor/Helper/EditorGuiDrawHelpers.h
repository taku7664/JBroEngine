...
namespace EditorGuiDrawHelpers
{
	...
	bool DrawAddComponentMenu(CScene& scene, ObjectId entity);

	// 버튼 + 팝업 조합으로 컴포넌트 추가 UI 표시.
	bool DrawAddComponentButton(CScene& scene, ObjectId entity);

	// ── 오브젝트 추가 ──────────────────────────────────────────────────────────

	// "Add Object" / "Add Child Object" MenuItem 하나를 표시.
	//   parent == INVALID_OBJECT_ID : 루트에 오브젝트 추가 → 레이블 "Add Object"
	//   parent != INVALID_OBJECT_ID : parent의 자식으로 추가 → 레이블 "Add Child Object"
	// 성공 시 생성된 엔티티를 선택 상태로 만들고 true 반환.
	bool DrawAddObjectMenu(CScene& scene, ObjectId parent);
