...
public:
    ...
    void Validate(const CGameScene& scene);
    ...
    CGameObject* OnDoubleClick(const CGameScene& scene, CGameObject* picked);

    // 더블 클릭 on 빈 공간:
    //   - m_context = m_context 의 부모 (루트면 INVALID)
    //   - 반환값 = 탈출한 엔티티 (호출자가 SelectEntities + FocusOnEntity에 사용)
    //             루트에서 탈출할 게 없으면 nullptr
    CGameObject* OnDoubleClickEmpty(const CGameScene& scene);
