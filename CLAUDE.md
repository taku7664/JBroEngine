...
- [tasks/physics-plan.md](./tasks/physics-plan.md) — 2D 물리 계획(D-199·D-203). 1~4 단계(커널 `JBroPhysics2D` 와 어댑터 `Physics2DSystem`)가 섰고 5 단계(에디터)·6 단계(질의 확장)가 남았다.
...
- [tasks/text-plan.md](./tasks/text-plan.md) — 2D 텍스트 계획(D-200). 결정은 섰고 코드는 1 단계부터 진행한다. 기존 엔진 `Text2D` 를 깨트려 본 결과
