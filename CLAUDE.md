...
- 재사용 가능한 헬퍼 함수나 객체는 따로 소스파일에 빼놓아 다른데서도 사용가능하게 한다.
...
- **0.0~1.0 정규화 값은 반드시 `SliderFloat` + 1.0 클램프.** 볼륨·불투명도·피벗처럼 범위가
  곧 작업 영역인 값은 슬라이더를 쓴다(현재 위치가 한눈에 보이고 아무 지점이나 바로 집는다).
  최대는 **1.0** 이다 — 2.0 같은 부스트 범위를 임의로 만들지 말 것.
  `DragFloat` 은 범위가 열린 값(좌표·크기·거리·PixelsPerUnit)에만 쓴다. 그런 값의 상한은
  "말도 안 되는 값 방지"용 클램프일 뿐이라 슬라이더로 만들면 조작이 불가능하다
  (0~10000 슬라이더는 1픽셀에 50유닛씩 움직인다).
