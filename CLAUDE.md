...
- [tasks/asset-plan.md](./tasks/asset-plan.md) — 에셋 시스템 계획(D-111·D-112). 1 단계(`Uuid`·레지스트리·`.jmeta`·스캔)와
  2 단계(타입별 풀·텍스처와 스프라이트 로드·해석 패스)가 섰다. 3 단계(텍스처 있는 스프라이트·UV 사각형·`SpriteLibrary`)가 다음이다.
  기존 엔진 에셋 시스템의 분석과 그쪽이 겪은 문제는 §1, 설계는 §2, 남긴 것은 §3 각 단계 끝과 §4
