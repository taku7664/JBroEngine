...
// ── CScriptModuleLoader ───────────────────────────────────────────────────────
// 사전 빌드된 GameCode DLL을 단순 로드/언로드합니다.
// LiveCompileManager와 달리 소스 감시나 컴파일을 수행하지 않습니다.
// 사용자는 GameCode 프로젝트를 따로 빌드한 뒤 에디터에서 DLL을 불러옵니다.
//
// DLL이 내보내야 하는 함수:
//   extern "C" __declspec(dllexport) IGameModule* CreateGameModule();
//   extern "C" __declspec(dllexport) void         DestroyGameModule(IGameModule*);
...
public:
	...
	// context.Reflection에 유효한 포인터가 있어야 스크립트가 등록됩니다.
