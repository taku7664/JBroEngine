...
//    계층(부모)은 Canvas 레벨 관심사이므로 여기서 다루지 않는다(CanvasSerializer 담당).
...
namespace Serialization
{
	...
	//   캔버스 직렬화(CanvasSerializer)는 계층을 평탄 목록+ParentIndex 로 다루므로 false 를 쓴다.
	YAML::Node WriteObject(const CGameObject& object, std::vector<AssetGuid>* referencedAssets,
	                       bool includeChildren = false);

	// YAML 맵에서 새 오브젝트를 만들어 canvas 에 추가하고 속성/Transform/컴포넌트를 복원한다.
	// Children 키가 있으면 자식 서브트리도 만들어 부모로 연결한다(복사 붙여넣기).
	// 최상위 오브젝트의 부모 연결은 호출자가 처리한다(CanvasSerializer 는 ParentIndex). 실패 시 nullptr.
