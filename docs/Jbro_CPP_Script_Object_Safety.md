...
> 채택한 것은 **값 타입 참조 · slot+generation · RefCount 없음 · 가짜 인스턴스 금지** 다.
> 대체된 절: **§1·§4 의 어휘, §6, §7, §9, §12, §13, §14**.
>
> 대체 요지 다섯.
> 1. **스크립트에 노출하는 참조는 `Ref<T>` 하나뿐이다.** 오브젝트 · 컴포넌트 · 스크립트 ·
>    에셋 · 캔버스가 같은 타입을 쓴다. 값 타입 핸들 클래스를 따로 만들지 않는다.
>    `GetComponent<T>()` 도 원시 포인터가 아니라 `Ref<T>` 를 반환한다.
> 2. **접근자는 하나다.** `Get()` 이 `T*` 를 돌려주고 `operator->` 가 그것을 부른다.
>    확인하고 쓰면(`if (T* p = ref.Get())`) 안전하고, 안 하면(`ref->`) 무효일 때 크래시다.
>    `Ref<GameObject>` 특수화만 `if` 없이 안전한 멤버(`Destroy()` 등)를 제공한다.
> 3. **무효 접근은 런타임 에러가 아니라 로그 후 무시다.** (안전 멤버 경로 한정)
> 4. **`World` 도 ECS 도 없다.** `Canvas` 가 오브젝트 풀과 타입별 컴포넌트 풀을 직접 소유한다.
>    `WorldHandle` / `ResolveWorld` / World 레지스트리 / `Scene` 은 어떤 형태로도 두지 않는다.
> 5. **엔진 내부 참조는 `SafePtr` 를 그대로 쓴다.** `Ref<T>` 는 스크립트 노출 표면 전용이며,
>    시스템은 풀 순회로 실체 참조를 받으므로 `Ref` 를 거치지 않는다.
>
> 식별자 3종: `InstanceId`(영속 uint64) · `InstanceHandle`(런타임 slot+gen) ·
> `InstanceRef`(둘을 담는 24B POD 저장부).
