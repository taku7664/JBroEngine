...
- **바뀐 것이 없으면 저장이 파일을 바이트 하나도 건드리지 않는다.** (MUST) (D-189)
  ...
  회귀는 **같은 내용을 거듭 적어 바이트 비교**로 잡는다. 키가 하나 더 붙는 것은 눈으로 보기 전까지
  드러나지 않고, 그때는 이미 사용자의 파일이 벌어져 있다.
...
- **오디오는 장치를 직접 열지 않고 `IPlatform::CreateAudioOutput` 을 거친다**(소켓과 같은 규약, 기본 null). 장치가 없으면
  ...
  - 버스 음량·음소거·솔로는 이펙트 노드 끝의 램프로 건다(D-204). 그룹 음량을 곧바로 바꾸지 않는다 - 딸깍 소리가 난다.
...
- 짧은 이름이 몇 개 늘어선 값(무시 패턴 등)은 `Widget::NameListEdit` 로 고친다. **한 줄이 한 이름**이고
  `SplitLines` / `JoinLines` 가 버퍼와 목록 사이를 옮긴다. (MUST) (D-189)
  **버퍼는 부르는 쪽이 든다** - 여러 줄 칸은 편집하는 동안 그 버퍼가 프레임을 넘어 살아 있어야 하고,
  매 프레임 목록에서 새로 지으면 커서와 선택이 풀린다. 빈 줄과 줄 끝 `\r` 은 이름이 되지 못한다.
- 목록에서 하나를 고르는 드롭다운은 `Widget::FilterCombo` 다. enum 은 `EnumCombo`, `AssetId` 는 `AssetField` 이고
  둘 다 그 위에 선다. 패널이 `BeginCombo`·`BeginPopup` 으로 고르는 목록을 직접 그리지 않는다. (MUST) (D-118)
- **지금 고를 수 없는 항목은 목록에서 빼지 않고 회색으로 두고, 왜 못 고르는지 툴팁으로 말한다.** (MUST) (D-180·D-181)
  찾던 이름이 사라지면 어디로 갔는지를 다시 찾게 된다. Enter 로 고르는 길도 보이는 첫 항목이 아니라
  **고를 수 있는 첫 항목**을 고른다.
...
- **편집은 캔버스 뷰에서, 시뮬레이션은 게임 뷰에서.** (MUST) (D-130·D-131)
  게임 뷰는 **게임의 카메라가 보는 화면**이고 기즈모도 피킹도 없다. 편집 화면(캔버스 뷰)은
  **편집 카메라**가 보는 화면이고 팬·줌·격자·선택 표시·기즈모가 거기 있다. 둘은 같은 프레임에
  함께 보이므로 뷰마다 렌더 타깃을 고른다(`CameraParams::target`).
  게임 뷰에서 편집하게 두면 카메라가 없는 캔버스를 편집할 수 없고, 게임이 카메라를 움직이는
  순간 편집 화면이 따라 흔들린다. 기존 엔진의 `CGameViewTool` / `CCanvasViewTool` 이 같은 나눔이다.
- **기즈모는 그림을 그린 카메라를 빌려 쓴다. 같은 행렬을 두 곳에서 세우지 않는다.** (MUST) (D-140)
  화면 좌표와 월드를 잇는 행렬은 렌더러에게 물어본다(`Renderer::GetLastEditorViewCamera`).
  에디터가 같은 계산을 한 번 더 하면 한쪽만 고쳐졌을 때 손잡이가 그림과 다른 자리에 서고,
  그 어긋남은 화면을 봐야만 드러난다. 편집 카메라는 **자기 렌더 타깃을 든 뷰**에서 나오며
  게임 뷰가 닫혀 있어도 기록한다 — 게임 뷰의 사정으로 편집 화면의 손잡이가 사라지면 안 된다.
- **에디터는 멈춘 채로 뜬다.** 재생을 누르기 전까지 스크립트·물리·네트워크는 돌지 않는다. (MUST) (D-131)
  돌면 방금 놓은 값이 다음 프레임에 덮어써진다. **멈춘다는 것은 갱신을 건너뛰는 것이 아니다** —
  트랜스폼과 렌더 추출은 그대로 돌아야 화면이 나온다. 정지는 **재생 전의 캔버스로 되돌리고**,
  되살릴 값을 먼저 뜨지 못하면 재생하지 않는다(§11.5).

- **검증 빌드는 솔루션 전체다. 에디터 화면은 실제 실행 파일로도 본다.** (MUST) (D-158)
  테스트 프로젝트만 빌드하면 테스트가 가진 포함 경로 덕에 통과하는 동안 에디터 실행 파일은 깨진 채로 남는다 -
  실제로 이틀 동안 그랬고, 사용자가 켠 에디터에는 그동안의 작업이 없었다. 테스트 창(숨긴 창·한 프레임 간격)이
  잡지 못하는 결함(누르는 시간·창 밖 뗌·기본 컴포넌트)은 실제 에디터를 띄워 조작해야 드러난다.
  공개 헤더는 ImGui 를 끌어오지 않는다 - 에디터 호스트는 ImGui 를 모른다.

- **에디터에서만 뜻이 있는 오브젝트 상태는 캔버스 파일에 남고, 게임으로 묶을 때는 빠진다.** (MUST) (D-163)
  오브젝트 플래그의 `EditorOnlyObjectFlags`(지금은 캔버스 뷰 숨김 하나)는 `.jcanvas` 의 `Flags:` 로 적히고, 패킹은
  `CanvasWriteMode::Package` 로 적어 그 비트를 지운다. 게임 뷰와 게임 실행은 그 비트를 보지 않는다 - 캔버스 뷰의
  그리기·집기만 본다.

- **떠 둔 것으로 되살리는 것은 보이는 상태를 전부 담는다. 새로 놓는 자리는 한 규칙이 정한다.** (MUST) (D-168)
  나무 스냅샷은 이름·활성·플래그와 **레이어**까지 담는다 - 하나라도 빠지면 되돌리기가 편집을 하나 더 한 것이 된다.
  새 오브젝트가 어느 레이어로 가는지는 `EditorActions::ResolveTargetLayer` 한 곳이 정하고(부모 → 고른 것 → 기본),
  자리와 레이어는 만들기 커맨드가 함께 받는다. 한 손짓은 실행 취소 한 번이다.

- **입력은 펌프가 아니라 꺼내 간 쪽이 비운다.** (MUST) (D-177)
  `IPlatform::PumpEvents` 는 쌓기만 하고, 받아 간 쪽이 `ClearInputEvents` 를 부른다. 한 프레임에 펌프가 두 번
  도는 자리(에디터)에서 펌프가 먼저 비우면 **아무도 보지 못한 입력이 사라진다** - 빠르게 친 글자가 빠졌다.
  꺼내 가는 쪽이 없으면 엔진이 프레임마다 비운다.

- **프로그램과 함께 놓인 파일은 실행 파일 기준으로 찾고, 파일 읽기는 플랫폼을 거친다.** (MUST) (D-176)
  글자 표·아이콘 글꼴처럼 배포에 함께 들어가는 것을 현재 작업 폴더로 찾지 않는다 - 어디서 띄웠는지에 따라
  있다가 없다. `IPlatform::GetExecutableFolder()` 기준으로 풀되, 실행 파일 옆에 없으면 준 경로 그대로 쓴다.
  경로로 파일을 여는 일은 `IPlatform` 이 한다. `fopen`·`fopen_s` 는 경로를 ANSI 로 보므로 한글이 든 경로에서 실패한다.

- **캔버스를 갈아 끼우는 길은 비우고·묻고·읽는다.** (MUST) (D-174)
  다른 캔버스를 열기 전에 `Canvas::Clear()` 로 비운다(읽기는 빈 캔버스에만 들어간다). 지난 캔버스를 가리키던
  고른 것·오브젝트 번호·되돌리기 더미·클립보드도 함께 비운다. **저장하지 않은 변경이 있으면 묻는다** - 사람이
  고른 대로 저장하거나 버리고, 저장이 실패하면 열지 않는다.

- **컴포넌트 타입은 갈래와 다중성을 등록할 때 댄다. 하나만 붙는 것을 두 번 붙이지 않는다.** (MUST) (D-180)
  기본은 `Multiple` 이고, 하나뿐이어야 하는 타입(트랜스폼·카메라·강체)이 등록 자리에서 `Single` 을 댄다.
  판정은 `ComponentRegistry::CanAttach` 한 곳이며, **막는 자리는 `AddComponentCommand`** 다 - 목록에서만 막으면
  붙여넣기와 다시하기가 그 옆으로 지나간다. 화면(인스펙터 드롭다운·오브젝트 메뉴)은 같은 판정으로 회색을 칠하고
  `이미 추가됨` 을 띄운다. 붙일 수 있는 목록은 `EditorActions::BuildAddComponentList` 하나가 만든다.

- **에디터 클립보드는 담는 것마다 따로 두고, 프로젝트를 닫을 때 비운다.** (MUST) (D-167)
  오브젝트 나무와 컴포넌트는 서로 다른 클립보드다 - 하나로 합치면 한쪽을 복사하는 순간 다른 한쪽이 지워진다.
  붙여넣기는 **커맨드를 거친다**. 들고 있는 값 안에 이 프로젝트의 오브젝트 번호와 에셋 번호가 있으므로
  `CloseProject` 가 둘 다 비운다.

- **프로젝트와 함께 사는 것을 한 번만 이어 두지 않는다.** (MUST) (D-165)
  에셋 시스템·캔버스·오브젝트는 프로젝트를 열 때 생기고 닫을 때 사라진다. 그것을 가리키는 에디터 쪽 캐시(그림·외곽선)는
  **프로젝트를 열 때마다 다시 잇고 닫을 때 끊는다**. 고른 것·오브젝트 번호·되돌리기 더미도 `CloseProject` 가 비운다 -
  닫는 길이 여럿이면 비우는 곳도 여럿이 되고, 그중 하나를 잊으면 다시 연 프로젝트에서 죽은 것을 만진다.

- **실행 루프가 멈추면 까닭을 남긴다. 시간이 흐르지 않은 프레임은 실패가 아니다.** (MUST) (D-160)
  호스트는 끝날 때 마지막 프레임 상태를, 엔진은 창이 닫혔는지 끝내기 요청인지를 적는다. 에디터 UI 는 0 초
  프레임을 아주 작은 시간으로 받는다 - 거절하면 첫 프레임에 에디터가 꺼진다. 음수와 NaN 은 여전히 거절한다.
  사용자에게 보이는 실패는 코드로 받아 번역된 문장으로 보이고, 엔진의 영어 문장은 로그에 둔다.

- **그림은 비율을 지켜 칸에 넣는다. 파일을 여는 창은 열 때마다 앞으로 나온다.** (MUST) (D-159)
  미리보기·아이콘·뷰어는 `Widget::FitInside` 로 크기를 잡는다. 칸에 늘여 붙이면 가로로 긴 시트가 찌그러진다.
  뿌리 탭의 창(스프라이트 뷰어)은 이미 떠 있어도 여는 손짓마다 포커스를 받는다 - 메인 탭 뒤에 가려진 채
  탭만 더하면 그 손짓이 아무 일도 하지 않은 것처럼 보인다.

- **겹쳐 그리는 좌표는 그린 화면의 크기로 센다.** (MUST) (D-150)
  편집 화면 텍스처는 패널보다 크게 잡히고, 엔진은 그 텍스처 전체를 화면으로 보고 그린다.
  패널 크기로 좌표를 세면 월드 원점을 패널 한가운데로 잡아 그림과 어긋난다.
  **이 어긋남은 코드로 드러나지 않는다** — 그리는 쪽과 재는 쪽이 같은 셈을 쓰므로 집어도
  집히고 끌어도 끌린다. 그래서 확인은 **화면 픽셀을 읽어** 그림과 겹쳐 그린 것의 자리를
  견주는 것으로 한다.

- **인스펙터의 머리도 편집이다.** (MUST) (D-142)
  활성과 사용 토글, 이름 고치기는 모두 커맨드를 거친다. 값을 그대로 쓰면 화면에서는 같아
  보이지만 되돌릴 수 없고, 되돌릴 수 없는 것은 편집이 아니다(§11.5).
  활성은 **고른 것 전체**에 간다 — 하나만 꺼지면 나머지가 그대로라 무엇이 바뀌었는지 알 수 없다.
...
- **도크는 두 겹이고 메뉴도 그 결을 따른다.** (MUST) (D-134)
  ...
  뿌리 노드에는 이름표를 달아 도구 창이 거기 붙지 못하게 한다 — 붙으면 메뉴 막대와 나란히
  서고 다시 넣을 방법이 화면에 없다. 안쪽 도크에는 달지 않는다: 패널의 이름표는 그 패널이
  처음 그려질 때에야 정해져서, 배치를 잡는 첫 프레임에 노드가 전부 뱉어 낸다.
  **배치는 도크 공간을 내기 전에 잡는다.** 내고 나서 잡으면 그 프레임의 노드가 빈 채로 굳는다.
  메뉴는 **프로젝트에 대한 것**(열기·저장·종료)이 뿌리, **지금 연 캔버스에 대한 것**
  (시뮬레이션·편집·창)이 메인 도크다. 기존 엔진의 `CRootDockWindow` / `CMainDockWindow` 와 같다.
- **눌러도 아무 일도 없는 메뉴 항목을 두지 않는다.** (MUST)
  기능이 아직 없으면 항목도 없다. 회색으로 둘 수 있는 것은 "지금은 못 하는 일" 이지
  "만들지 않은 일" 이 아니다 — 없는 것을 그려 두면 고장으로 읽힌다.
- **단축키는 표 하나에서 온다.** (MUST) (D-132)
  누르는 자리·메뉴에 보이는 글자·할 수 있는지 재는 자리가 갈리면, 메뉴에서는 못 하는데 키로는 되는
  자리가 생기고 키를 바꿔도 화면은 옛 글자로 남는다. `EditorShortcuts` 가 그 표이며
  단축키 참조 창도 같은 표를 읽는다 — 손으로 적은 목록을 따로 두지 않는다.
- **패널이 오브젝트에 하는 일은 공용 동작을 거친다.** (MUST) (D-132)
  오브젝트 추가·복사·붙여넣기·삭제·부모 해제는 계층·캔버스 뷰·메뉴·단축키가 모두 한다.
  `EditorActions` 가 그 한 벌이고 **회색으로 보이는 규칙까지** 거기 있다. 패널 안에 같은
  메뉴를 다시 쓰지 않는다.
- **엔진이 사람에게 하는 말은 로그로 간다.** (MUST) (D-133)
  창으로 띄운 에디터에는 콘솔이 없다. `std::printf` 로 낸 알림과 경고는 그대로 사라지므로
  `JBro/Core/Log.h` 를 거치고, 로그 창이 그것을 보인다.
