    ...
    bool EditorUI::Draw(IRHICommandContext& commands)
    {
        ...
        for (int listIndex = 0; listIndex < drawData->CmdListsCount; ++listIndex)
        {
            ...
            for (int commandIndex = 0; commandIndex < list->CmdBuffer.Size; ++commandIndex)
            {
                ...
                // 화면 밖까지 태연히 내보내고, D3D12 는 음수 시저를 받지 않는다.
                //
                // 이 두 줄을 지워도 화면은 똑같이 나온다 - 하드웨어가 알아서 0 으로
                // 붙여 버리기 때문이고, 뮤테이션 테스트로 확인했다. 드러나는 곳은
                // 디버그 레이어뿐이다. 그래도 남긴다: 유효하지 않은 값을 넘기지 않는
                // 것이 계약이고, 다른 백엔드나 드라이버가 같이 봐 줄 보장은 없다.
                if (scissor.left < 0)
                {
                    scissor.left = 0;
                }
