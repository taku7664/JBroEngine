...
// 손대면 화면이 달라진다(ProjectRule §11.1).
//
// ImGui 내부 API 를 쓴다. 그래야 행 사각형을 돌려줄 수 있다.
...
namespace JBro::Widget
{
    namespace
    {
        // `TreeBegin` 이 이름표 자리를 내준 뒤에도 커서가 제자리로 돌아와야
        // 다음 줄이 어긋나지 않는다. 중첩될 수 있으므로 스택이다.
        struct TreeCursorRestore
        {
            ImVec2 CursorPos;
        };
    ...
    {
        ImGuiWindow* window = GetCurrentWindow();
        IM_ASSERT(g_cursorRestoreStack.Size > 0);
        if (g_cursorRestoreStack.Size <= 0)
        {
            return;
        }
        const TreeCursorRestore restore = g_cursorRestoreStack.back();
        g_cursorRestoreStack.pop_back();
