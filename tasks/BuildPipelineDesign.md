...
4. `emcc @BuildScripts/Web/web_game_sources.txt` 로 `index.html`, `.js`, `.wasm` 산출물을 package root 에 생성합니다.
5. Emscripten preload 에 package `Content` 를 `/Content` 로 싣습니다.
6. 검증 시 `index.html`, `.js`, `.wasm`, manifest, pack 존재를 확인하고 `GameScript.dll`, loose `Content/Assets`, `SDK`, `Editor`, `Localization` 은 금지합니다.

현재 Web 첫 단계의 한계:

- 사용자 script static link 는 아직 별도 단계로 남아 있습니다.
