...
3. Metal 또는 WebGPU 대체 전략 결정
   - iOS browser WebGPU 와 native WebGPU 는 별개로 봐야 합니다.
   - native iOS 는 Metal backend 가 현실적인 우선순위입니다.
...
- 단기: OpenGL ES 또는 Vulkan 중 팀/코드베이스 부담이 낮은 쪽 선택
- 장기: RHI 통합 관점에서 Vulkan/Metal/WebGPU 계열로 수렴

iOS:

- Metal backend 가 현실적인 기본값입니다.
...
- Metal RHI backend 또는 선택한 iOS backend 구현
...
Android 는 Vulkan/OpenGL ES/WebGPU 중 선택이 필요하고, iOS 는 Metal 을 피하기 어렵습니다.
...
5. iOS 는 signing/Metal 비용이 크므로 Android 의 mobile lifecycle/filesystem/pack contract 가 안정된 뒤 들어갑니다.
