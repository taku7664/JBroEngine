...
1. **`[완료]` 레지스트리와 메타** (2026-09-18, `ffdddb3`·`ade7e2a`): `Uuid`(§2.1), `AssetType` 열거와
   ...
   **남긴 것**: ~~`SaveAssetMetaFile` 은 네 키만 적으므로 옵션이 있는 메타를 덮어쓰면 옵션이 사라진다~~ → D-120 에서
   `AssetMetaFile` 이 두 옵션 블록을 리플렉션으로 읽고 써 닫았다.
...
4. **`[완료]` 에디터** (2026-09-20) - 결정은 D-116·D-117, 순서와 항목은 `todo-2d.md` 의 "에셋 4 단계"(여섯 걸음 전부 섰다:
   1·2 가 D-118, 3·4 가 D-119, 5 가 D-120, 6 이 D-121). 요약: 에셋 필드는 공용 드롭다운 위젯
