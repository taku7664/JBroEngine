...
- `[완료]` **12. 에셋 브라우저**(D-139). 두 칸(폴더 나무 | 내용), 길잡이 줄, 새 폴더·이름 바꾸기·
  ...
  `[열림]` **아이콘 보기와 다중 선택**은 남았다. 아이콘 보기는 텍스처 썸네일을 에디터가 얻는 길이
...
- `[완료]` **3D 의 캔버스 뷰**(D-136). 궤도 카메라(오른쪽 끌기로 돌고 휠로 거리)와 원근 투영으로
  선다. 게임 카메라가 없어도 그린다.
  `[열림]` **3D 의 기즈모·격자·선택 윤곽**은 아직 없다. 화면 좌표와 월드를 잇는 것이 2D 처럼
  나눗셈 하나가 아니라 이번 프레임의 뷰-투영이 필요한데, **렌더러가 편집 카메라를 내주는 길이
  없다.** 여기서 같은 행렬을 한 번 더 세우면 엔진이 세우는 것과 둘로 갈려, 한쪽만 고쳐졌을 때
  손잡이가 그림과 다른 자리에 선다. `GetLastViewCamera` 의 편집 뷰 판이 먼저 있어야 한다.
