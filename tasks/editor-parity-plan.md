...
- `[열림]` **스프라이트 알파 외곽선**(기존 `CCanvasViewContour`, 408줄). 선택한 스프라이트의
  실제 모양을 따라 윤곽을 그린다. **막혀 있지 않다** - `TextureData::pixels` 가 CPU 에 있어
  알파 경계를 따라갈 수 있다(D-147 의 썸네일이 같은 픽셀을 읽는다). 남은 것은 경계 추적과
  그 결과를 캐시하는 일이고, 사각형 윤곽이 있는 지금도 고르고 옮기는 데는 지장이 없다.
