...
| 에셋 브라우저의 정렬과 열 | 정렬 기준 칸(이름·종류·수정한 날짜), 목록 보기는 네 열 표(이름·종류·수정한 날짜·GUID) | 경로순 하나, 한 줄에 `이름  종류` | **완료** (D-196). 줄은 트리 줄 그대로 두고 그 안에 열을 맞춰 그린다 |
| 스프라이트 임포트 옵션의 칸 | 자르는 방식에 맞는 칸만 보인다(칸 수 → 행·열 수, 칸 크기 → 폭·높이, 여백·간격은 격자일 때만). 줄마다 설명 툴팁(`ImporterGui::DrawLocalizedRow`) | 리플렉션이 필드를 전부 그려 `None` 인 그림에도 `rowCount`·`cellWidth`·`gapX` 가 보인다. `FieldAttributes::Tooltip` 은 있지만 어느 필드도 쓰지 않고, 쓰더라도 글자를 그대로 보여 로컬라이징을 거치지 않는다 | **열림 - 결정이 필요하다.** 둘 다 리플렉션 속성(JBroCore)의 설계를 건드린다: 보이는 조건을 속성으로 둘지(`VisibleWhen`), 스프라이트 옵션만 손으로 그릴지, 툴팁을 로컬라이징 키로 정할지 |
...
**Windows 에서 빌드와 테스트를 한 번 돌려야 끝난다.** → 끝났다(2026-09-25). 솔루션이 경고 없이 서고
`JBroTests` 가 통과하며, 음성 검사 셋이 MSVC 로 지은 실행 파일에서도 전부 실패한다(D-195 항목에 적었다).
