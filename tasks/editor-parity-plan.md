...
- `[완료]` **에디터 세션**(D-146). 보던 캔버스·편집 카메라·에디터 언어가 프로젝트 파일에
  ...
  `[열림]` 창 배치(ImGui `.ini`)는 기본 배치를 잡는 `DockBuilder` 와 겹쳐 아직 넣지 않았다.
- 인스펙터의 기능 차이(3,068줄 대 1,183줄). 레이아웃은 사용자가 좋다고 했으므로
  **무엇이 없는지만** 따로 재서 여기 적는다. 재어 보니 다음과 같다(D-142).
  ...
  `[열림]` **필드 편집을 고른 것 전체에 델타로.** 기즈모 쪽은 이미 그렇게 한다(D-109).
  인스펙터의 필드는 주된 것 하나만 고친다.
...
- `[열림]` **스프라이트 알파 외곽선**(기존 `CCanvasViewContour`, 408줄). 선택한 스프라이트의
  실제 모양을 따라 윤곽을 그린다. 텍스처의 픽셀을 에디터가 읽는 길이 없어 막혀 있다.
