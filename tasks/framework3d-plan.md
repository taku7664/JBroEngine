...
## 2.10 렌더러 성능 실측 (2026-09-18, D-110)

세 백엔드에 같은 장면을 던지는 벤치마크(`JBRO_BENCH=1 JBroTests.exe`, Release, RTX 2080, 1280x720 오프스크린,
`Immediate` 제시, 120 프레임 평균)로 병목을 찾고 고쳤다. 숫자는 프레임 벽시계 시간(ms)이고 "end" 는 `EndFrame`
(업로드·기록·제출·제시) 쪽이다. **Debug 빌드의 숫자는 쓰지 않았다** - 첫 회차를 Debug 로 재고 배열 경계 검사가
잰 것을 봤다.

| 장면 | D3D12 전 → 후 | D3D11 전 → 후 | Vulkan 전 → 후 |
|---|---|---|---|
| 빈 프레임(클리어만) | 0.14 | 0.07 | 0.16 |
| 스프라이트 60000 | 0.88 → 0.61 | 0.82 → 0.54 | 1.18 → 0.87 |
| 메시 16000(한 종류) | 0.95 → 0.43 | 0.71 → 0.42 | 1.52 → 0.57 |
| 메시 16000(네 종류 섞음) | 1.09 → 0.41 | 0.75 → 0.38 | 1.57 → 0.56 |
| 텍스처 드로우 1000(같은 텍스처) | **실패** → 0.16 | (DXBC 없음) | 0.22 → 0.24 |
| 텍스처 드로우 1000(둘 번갈아) | **실패** → 0.16 | (DXBC 없음) | 0.36 → 0.26 |

- **메시마다 드로우 하나였다.** 업로드가 뷰 안에서 메시 슬롯별로 세고(계수 정렬, O(n)) 그 자리에 흩뿌려 같은 메시를
  이어 놓고, 기록은 메시 **종류**마다 드로우 하나를 낸다(`MeshRun`). 16000 개 정육면체가 드로우 하나다. 같은 메시
  안에서는 제출 순서가 지켜진다. 뷰가 여럿이면 뷰마다 따로 모인다.
- **D3D12 는 텍스처를 묶는 드로우 512 개에서 나머지를 조용히 버렸다.** 드로우마다 디스크립터를 복사해 프레임 몫
  (샘플러 512)을 다 썼기 때문이다 - ImGui 는 드로우마다 같은 폰트 아틀라스를 묶으므로 무거운 에디터 화면이 그 길에
  있었다. 컨텍스트가 이 프레임에 스테이징한 테이블 16 개를 기억해 같은 묶음이면 복사 없이 다시 걸고, 텍스처 몫은
  4096 으로 늘렸다. Vulkan 도 같은 모양으로 set 16 개를 기억한다(번갈아 묶는 드로우 0.36 → 0.26).
- **인스턴스 배열을 항목마다 `Add` 로 채우던 것이 60000 개에서 0.3ms 였다** - 자료를 옮기는 memcpy(2.6MB, 0.1ms)의
  세 배. 크기를 한 번 잡고 자리에 쓴다.
- **Vulkan 의 남은 차이는 `vkQueuePresentKHR` 다.** 계측 결과 빈 프레임에서 0.15ms, 스프라이트 프레임에서 0.40ms 를
  제시 호출 안에서 보낸다 - 이 드라이버는 제시가 앞 프레임의 GPU 작업을 기다린다. 펜스 대기는 0 이다(GPU 에 매이지
  않았다). D3D 의 플립 모델 `Present` 는 그 시간을 감춘다. 렌더러가 고칠 자리가 아니라 적어 둔다. `[열림]`
- **시도했다가 되돌린 것**: Vulkan 업로드 버퍼를 DEVICE_LOCAL|HOST_VISIBLE(BAR)에 두기 - 스프라이트 60000 이 1.19 →
  1.26 으로 나빠졌다. 시스템 메모리에 그대로 둔다.
- **테스트**: 계약 테스트(같은 핸들 다섯 → 드로우 둘, 낡은 핸들은 셈만), 텍스처 바인딩 테스트(드로우 사이에 텍스처를
  바꾸면 마지막 것이 남는다 - 재사용이 텍스처를 열쇠로 하는지). 뮤테이션 7/7(드로우 인스턴스 수·첫 인스턴스·흩뿌리기 커서·버림 셈·스프라이트 알파·두 백엔드의 캐시 열쇠) - 잡는 테스트가 모두 스위트 앞쪽이라 한 변이에 30 초 남짓 걸렸고, 하나를 따로 돌려 3 분(복구 빌드 포함)인 것을 대조해 가짜가 아님을 확인했다.

## 2.11 그래픽스 계층 코드 리뷰 (2026-09-18)

렌더러·D3D12·D3D11 을 검토자 셋이 따로 읽었다(Vulkan 은 §3 의 앞 회차). 고친 것과 남긴 것.

- **스프라이트가 깊이 패스 위에 있을 때 파이프라인 포맷이 어긋났다**(CONFIRMED). 메시가 있는 뷰는 깊이 첨부를
  달고, 스프라이트 파이프라인은 깊이 포맷을 모른 채 만들어져 세 API 모두 규약 위반이었다. RHI 파이프라인에
  `depthTest`·`depthWrite` 를 두고, 깊이 포맷만 같고 둘을 끈 쌍둥이 파이프라인을 깊이 패스에서 쓴다. 세 백엔드
  픽셀 테스트(정육면체 위 스프라이트, 검증 0)가 지킨다.
- **`EndFrame` 안의 배열 확장이 예외를 낼 수 있었다**(CONFIRMED) - 프레임이 열린 채로 빠져 이후 영원히
  `InvalidState`. `RecordViews` 를 잡아 `AbortFrame` 으로 보낸다.
- **인스턴스 배열을 프레임마다 `Resize` 하던 것은 값 초기화(memset)였다** - 16000 개에 1.3MB. 초기화 때 상한 크기로
  한 번 잡고 개수만 든다. 메시 히스토그램은 `RegisterMesh` 에서 자란다(프레임 안 할당 금지).
- **D3D12 되읽기가 펜스 값을 따로 셌다**(CONFIRMED). 다음 프레임이 같은 값을 신호해 아직 도는 프레임의 할당자를
  되감을 수 있었다. 프레임과 같은 계수기를 쓴다.
- **D3D12 `AbortFrame` 이 기록만 된 배리어의 상태를 남겼다**(CONFIRMED). 프레임 시작 상태를 떠 두고 되돌린다.
  스모크 테스트(버린 프레임 뒤 프레임이 검증 0)가 지킨다. 깊이 텍스처를 샘플링할 SRV 포맷도 `R32_FLOAT` 로.
- **D3D11**: 푸시 상수 버퍼를 DYNAMIC + `Map(DISCARD)` 로(드로우마다 `UpdateSubresource` 는 느린 길), 컨텍스트가
  그 버퍼의 참조를 든다(패스 중 파이프라인 파기에도 안전), 상수 버퍼 일부 쓰기는 거절(꼬리를 0 으로 만들던 것),
  `WriteTexture` 는 프레임 안에서 거절(계약), 크기 바꾸기 앞에 `Flush`, 실패한 크기 바꾸기·제시는 `SurfaceLost` 로
  남아 다음 크기 바꾸기가 풀어 준다, 팩토리는 디바이스의 어댑터에서, 테어링과 Alt+Enter 는 D3D12 와 같이, 긴
  정보 큐 메시지를 오류로 세던 것 수정, 패스 시작에 첨부 전체 시저·뷰포트.
- **남긴 것** `[열림]`: 깊이 텍스처를 프레임 슬롯마다 두지 않는다(D3D 는 렌더 타깃 쓰기가 제출 순서로 정렬되고,
  Vulkan 은 같은 레이아웃 전이가 배리어를 내지 않아 패스 사이 쓰기 순서가 규약상 열려 있다 - 검증 레이어는
  조용하다); D3D11 의 제시 전 사본(되읽기용) 매 프레임 복사; 플립 모델 스왑체인의 sRGB 포맷(세 백엔드 공통 계약
  구멍); `LoadOperation::Discard` 를 D3D11 이 무시; 뷰포트 소수 자리는 바깥으로 넉넉히 자른다.
  뮤테이션 1/5. 살아남은 넷은 지금 테스트가 관측할 수 없는 것들이다: 깊이 패스 위 스프라이트 파이프라인의 포맷
  불일치(두 디버그 레이어가 이 기계에서 말하지 않았다 - D3D12 는 깊이를 끈 PSO 의 DSV 포맷 UNKNOWN 을 허용하고,
  Vulkan 레이어는 조용했다; 규약 위반이라 고친 것은 그대로 둔다), D3D12 `AbortFrame` 의 상태 되돌림(GPU 기반 검증
  없이는 배리어 생략이 보이지 않는다), D3D11 상수 버퍼 일부 쓰기(부르는 코드가 없다), 그리지 않은 프레임의 카메라
  (게임 뷰 패널을 가린 채 기즈모를 재는 에디터 테스트가 없다). **뮤테이션 도구의 시험 시간 한계를 900 초로 올렸다** -
  스위트가 4.5 분이 되어 180 초 한계에 살아남은 변이가 전부 "hang" 으로 나왔고, 그 회차는 결과가 아니었다.
