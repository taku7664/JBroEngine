...
- **JBro 기능은 전부 내장 확장으로 만들고, 코어 패치는 기능이 아니라 제품 모양에만 쓴다.**
  **패치는 넷이다(2026-09-16 확정, §5.2 의 패치 목록).** 0001 첫 실행 한국어, 0002 채팅·AI 제거,
  0003 `.vsix` 설치 차단, 0004 메뉴 구조. 0004 의 세부 항목만 남았다.
...
| 코어 패치 | 한다. **무엇을 바꿀지는 2026-09-16 에 정했다**(§5.2 의 패치 목록) |
| 확장 마켓플레이스 | 연결하지 않는다. **Code-OSS 가 `extensionsGallery` 키를 갖고 있지 않아 비울 것이 없다**(2026-09-16, §5.3) |
| 식별자 | CLI 명령어 `jbro-script-editor`, 설정 폴더 `.jbro-script-editor`, 리포 `JBroScriptEditor`, 로컬 위치는 F: |
| 편집기 리포의 원격 | 2026-09-15 에 Claude 세션이 로컬 git 리포로 만들었고 원격은 없다. **2026-09-17 빡대리 확인: 그대로 두고, GitHub 연결은 나중에 빡대리가 한다** |
...
따로 로그를 남기므로 이것이 증거가 되지 못한다. 패치 0002 를 적용하면 에이전트 호스트가 뜨지 않는다.
...
| 0002 | `remove-chat-ai` | **적용** | `contrib/chat/browser/chat.shared.contribution.ts:2379` 의 `chat.disableAIFeatures` 기본값을 `true` 로 바꾼다 | 채팅·AI 기능과 그것이 띄우는 에이전트 호스트를 기본으로 끈다. 사용자는 설정에서 켤 수 있다. 아래 "0002" 참고 |
...
**채택한 방식: `chat.disableAIFeatures` 기본값을 `true` 로(2026-09-17 실측, 같은 날 채택).** 한 줄이다. 띄워서 잰 결과:
...
- **화면에 채팅 UI 와 Copilot 안내가 뜨지 않는다**(빡대리가 화면으로 확인)

**없애지 않고 기본으로 끈다(2026-09-17 결정).** 코드는 남고, 사용자가 설정에서 켜면 돌아온다. 2026-09-16 의
"없앤다" 를 이 뜻으로 정했다. 한 줄이라 upstream 을 올릴 때 부담이 거의 없는 것이 이유다. 사용자가 켜지 못하게
막는 추가 패치는 하지 않는다. 필요해지면 그때 방법(정책으로 고정하는 길 등)을 조사한다.
...
- **명령 팔레트에서 "VSIX" 를 검색해도 설치 항목이 나오지 않는다**(빡대리가 화면으로 확인). 확장 뷰 메뉴·탐색기 우클릭·끌어놓기는 화면으로 보지 않았다. `installFromLocation` 의 거절은 부를 입구가 남아 있지 않아 실행으로 재지 않았다.
...
- ~~**YAML 연결은 에디터 안에서 확인하지 않았다.**~~ **확인했다(2026-09-17).** 패치를 적용한 개발 빌드에
  `--extensionDevelopmentPath` 로 확장을 올리고 `F:\Project\Test.Jproject` 를 열어 언어가 YAML 로 잡히는 것을 빡대리가 봤다
  (확장자 대소문자가 달라도 잡힌다).
...
**화면 확인(2026-09-17).** 같은 개발 빌드에 확장을 개발 모드로 올려 `test/snap/enemy.jscript` 를 열었고, 의도대로 칠해지는
것을 빡대리가 봤다. 개발 모드로 읽어 들이는 것은 설치가 아니므로 패치 0003 에 걸리지 않는다. 방법:
`scripts\upstream-launch.cmd --extensionDevelopmentPath=<확장 폴더> <파일>`.
...
- 패치 0001~0004 를 적용한다(§5.2 의 패치 목록).
...
1. ~~코어 패치로 무엇을 바꿀 것인가.~~ **정했다(2026-09-16). 패치 넷이다(§5.2 의 패치 목록).**
   0001 첫 실행 한국어, 0002 채팅·AI 제거, 0003 `.vsix` 설치 차단, 0004 메뉴 구조.
   **0004 의 세부 항목만 미정이고**, 포크를 띄워 실제 화면을 보고 정한다. 기본 레이아웃은 유지한다.

   같은 날 목록에서 뺀 것 셋:

   - **시작 화면(환영 탭).** 뜨지 않게만 하면 되고, 그것은 확장이 `workbench.startupEditor` 의 기본값을
     `none` 으로 기여하면 된다(`platform/extensions/common/extensions.ts:217`). 그 자리에 JBro 만의
     시작 화면을 넣기로 하면 그때 다시 패치 후보가 된다.
   - **계정 메뉴.** 깃 로그인을 연결하기로 했으므로 그대로 둔다. 채팅을 없애도 Git·GitHub 확장이
     저장소에 접근하려면 이 메뉴가 로그인 창구다.
   - **`inlineCompletions`.** 로그인과 무관한 일반 틀이고 지워서 얻는 것이 없다(§5.2).
...
| `jbroc` 렉서·파서 | **완료**(2026-09-17, jbroc-rules §11). 명령줄 `JBroc` 실행 파일은 아직 없다 | — | — |
...
| 포크 빌드(브랜딩·설치본) | 시작 전. **upstream 을 빌드해 띄웠고 패치 0001~0003 을 적용했다**(2026-09-17, §4.2·§5.2) | 중간 | 0004 의 세부(§7), 릴리스 빌드가 내장 Copilot 확장을 묶어 넣는지(§5.2 의 "0002"), 첫 실행이 영어로 뜨는 문제(§5.5) |
