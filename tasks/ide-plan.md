...
- **한국어 UI 가 없다.** VS Code 의 한국어 화면은 본체가 아니라 언어 팩 확장이 준다.
  마켓이 없으면 편집기가 그 팩을 직접 실어야 한다(§7 Q2).
...
**완료 조건**: 네 결과가 이 문서의 §3·§4 에 실측으로 기록되어 있다.
...
2. **편집기 화면을 한국어로 낼 것인가.** 씬 에디터는 기본이 `ko-KR` 이다(D-80). 편집기도 맞추려면
   한국어 언어 팩을 내장 확장으로 싣는다. 싣지 않으면 메뉴·설정 화면이 영어로 나온다.
