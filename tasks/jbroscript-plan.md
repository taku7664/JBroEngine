...
| 참조 | C++ 참조(`&`)는 없다. **`ref` 만** 있다. `ref T` 는 포인터이고 null 검사가 된다. `ref const float` 는 `const float*` 다 |
...
**Q1. `ref` 가 원시 포인터면 `ProjectRule.md` §6 의 MUST 와 부딪힌다.**
그 규칙은 "스크립트가 대상의 수명 밖까지 보관하는 멤버에 raw pointer 를 저장하지 않는다", "스크립트 참조는
`GameObjectHandle` 과 `Ref<T>` 둘뿐" 이다. 필드로 저장한 원시 포인터는 대상이 파괴되거나 핫 리로드되면
그대로 매달린다.
**제안**: 문법은 `ref` 하나로 두고, **내려가는 C++ 를 대상에 따라 고른다.** 사용자는 "null 이 될 수 있는 참조" 하나만 안다.

| `ref` 의 대상 | 쓸 수 있는 자리 | C++ | null 검사 |
|---|---|---|---|
| `script`, 빌트인 컴포넌트 | 필드·지역·매개변수 | `Ref<T>` | 된다 |
| 게임 오브젝트 | 필드·지역·매개변수 | `GameObjectHandle` | 된다 |
| `class` | 필드·지역·매개변수 | `SafePtr<T>`(대상이 죽으면 null) | 된다 |
| 값(`float`, `struct` 등) | **매개변수·지역만** | `T*` / `const T*` | 된다 |

값에 대한 `ref` 를 필드에 못 두는 것 하나만 막고, 나머지는 빡대리의 뜻(`ref` = null 검사 되는 포인터)을 그대로 따른다.
그러면 §20.3 의 `out` 은 필요 없어진다 — `Raycast(..., ref hit)` 처럼 `ref` 매개변수가 그 일을 한다.
