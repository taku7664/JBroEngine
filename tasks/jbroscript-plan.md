...
|---|---|---|---|---|
| `script` | 뜬다 | private | 없다 | (§21.4 Q4) |
| `class` | 안 뜬다 | private | **있다**(2026-09-15 수정, 처음에는 "없다") | (§21.4 Q4) |
| `struct` | 안 뜬다 | public | 있다 | **그렇다**(private 도 프로퍼티) |
| `interface` | 안 뜬다 | public | — | — |

빡대리가 준 예시(원문 그대로. `struct` 예시의 첫 줄이 `class MyStruct` 로 적혀 있어 §21.4 Q6 으로 확인한다):
...
**Q4. 프로퍼티 노출.** `struct` 는 기본으로 전부 프로퍼티다. `script`·`class` 는 §12.3 의 대괄호 규칙을 유지하는가,
아니면 `public` 이 노출을 뜻하는가(Unity 방식)? `struct` 에서 하나를 빼려면 `[noserialize]`/`[hidden]` 을 쓰는가?
...
**Q6.** `struct` 예시의 `class MyStruct` 는 `struct MyStruct` 의 오타인가?
