...
| 함수 접미사 | `callback`: 이 함수는 불려진다(콜백)는 표시. `require`: 순수 가상 함수 |
| 상속 | C++ 처럼 다중 상속을 지원한다. **다이아몬드 상속은 금지**한다 |
...
- **같은 `interface` 를 두 경로로 받는 것도 다이아몬드다**(`A : I, B` 이고 `B : I`). C#·Java 에서는 흔히 허용되는
  모양이라 걸리기 쉽다(§21.4 Q7).
...
**Q5. `callback` 의 뜻.** (가) 엔진이 부르는 훅(`OnUpdate` 등)을 재정의한다는 표시인가, (나) 이벤트·델리게이트로
등록되어 불리는 함수라는 표시인가? (가)라면 사용자 `class` 의 **몸통 있는 가상 함수**를 자식이 재정의할 때는
무엇으로 표시하는가(`require` 는 순수 가상뿐이다)?

**Q6.** `struct` 예시의 `class MyStruct` 는 `struct MyStruct` 의 오타인가?

**Q7.** 같은 `interface` 를 두 경로로 받는 것도 금지하는가(§21.3)? 허용하려면 C++ 가상 상속이 필요하고, 그 비용과 규칙을
사용자는 몰라도 되지만 이미터와 ABI 는 알아야 한다.

### 21.5 아직 없는 것 — 정해야 문법이 닫힌다

1. **`class` 인스턴스의 수명.** 값으로 못 쓰니 `ref` 로만 다루는데, **누가 만들고 누가 지우는가**가 없다. 엔진 규칙
   (고유 소유는 `OwnerPtr`, 비소유는 `SafePtr`, `new`/`delete` 금지)에 맞추면 `own MyClass`(소유) + `ref MyClass`(비소유) +
   생성식 하나가 된다. 없으면 사용자가 해제를 직접 챙겨야 하고 그것이 곧 "C++ 특유의 지식" 이다.
