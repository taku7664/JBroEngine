...
- **자원은 켤 때 잡고 끌 때 돌려준다(D-125).** 엔진은 `NetworkHost` 를 늘 세운다 - 스크립트가 서비스로 네트워크를 켤 수
  있어야 하므로 부를 대상이 있어야 한다. 그러나 **역할을 잡기 전까지는 아무것도 잡지 않는다.** 트랜스포트의 공용 버퍼는
  `Listen` / `Connect` / `HostPeers` / `ConnectPeer` 가 잡고 `Close` 가 돌려주며(이벤트 큐만은 마지막 `Disconnected` 를 들고
  있으므로 소멸자까지 산다), 복제는 역할이 생긴 뒤에 그 역할이 쓰는 쪽만 선다 - 한 프로세스가 서버이면서 클라이언트일 수
  없기 때문이다. 캔버스를 묶는 것은 풀의 등록 순서를 기억하는 일까지이고, 복제가 설 때 그 순서 그대로 넘긴다.
  네트워크를 쓰지 않는 에디터와 단일 플레이가 이 예산을 지지 않게 하는 것이 이유다.
...
4. `[완료]` **복제 (A) 독립 단계.** (2026-09-20) `Replication/ReplicationTypes.h`(`NetworkObjectId`·`SpawnDesc` 32 B POD·메시지 대역
   ...
   **실측(§2.6 열림 항목의 답)**: 2000 오브젝트 × 8 B, 매 스텝 200 개 변경, Debug 빌드, 인메모리 소켓: 스텝당 **1.86 ms**
   ...
   dirty 비트 없이도 예산 안이다 - 넣지 않는다. **이 값은 클라이언트가 하나일 때의 것이다**(§3-8 이 그 조건을 달았다).
   테스트 +7: 깨끗한 와이어 수렴, 30% 유실·중복·재정렬 수렴, 소멸 전파, 늦게 온 클라이언트,
...
5. `[완료]` **부착.** (2026-09-20) 이 단계 전의 네 커밋에는 `source/JBroEngine/` 변경이 없다(`git log --stat` 로 확인).
   ...
   **자원을 잡는 때(D-125, 2026-09-20).** 처음에는 `EngineInstance` 가 `NetworkHost` 를 세우는 것만으로 트랜스포트 버퍼가,
   프레임워크가 캔버스를 묶는 것만으로 복제 서버와 클라이언트가 함께 섰다. 그래서 네트워크를 쓰지 않는 에디터와 단일 플레이도
   같은 값을 물었다 - 트랜스포트 **1250050 B**(테스트가 재서 찍는다)에 복제가 양쪽 합쳐 **약 40 MB**(기본 설정에서 스냅숏 이력
   32 틱 × (512 KB + 8192 × 16 B) = 20 MB 가 서버와 클라이언트 각각, 계산값이다). 지금은 둘 다 게임이 `StartServer` /
   `Connect` 로 켠 뒤에 서고, `Disconnect` 가 돌려주며, 복제는 역할이 쓰는 한쪽만 선다. `EngineConfig::networkEnabled` 는
   남아 있으나 이제 "호스트 객체와 스크립트 서비스를 두는가" 만 정한다 - 켜져 있어도 자원은 잡지 않는다.
   검증: 트랜스포트는 역할이 없을 때 `GetReservedBytes() == 0`, `Listen` 뒤 1 MB 초과, `Close` 뒤 다시 거의 0 이고 그때도
   `Disconnected` 는 남는다. 호스트는 캔버스를 묶고 프레임을 돌려도 `IsReplicating()` 이 거짓이며, `StartServer` 뒤 참이 되고
   연결 전에 등록해 둔 풀이 그대로 복제된다. 뮤테이션 둘(생성자에서 버퍼 잡기, `BindCanvas` 에서 복제 세우기) 모두 잡혔다.
...
8. `[완료]` **전수 검수와 그 결과.** (2026-09-20, D-126) 여섯 단계를 신뢰할 수 없는 입력이 들어오는 자리와 매 프레임 도는
   자리 위주로 다시 읽고 여덟 가지를 고쳤다.
   **상대가 보낸 것을 믿던 자리**: (1) `UnreliableSequenced` 의 "메시지 ID 별 최근 순번" 이 `Table` 이라 **상대가 고르는 키**로
   연결마다 6 만 항목까지 자랄 수 있었다 - ID 로 고른 고정 슬롯 64 개로 바꿨다(충돌하면 나중 것이 자리를 가지고, 그때 잃는 것은
   역전 폐기 한 번이다). (2) 데이터그램의 채널 바이트를 검증하지 않아 범위 밖 enum 이 게임까지 갔다 - `UdpProto::Decode` 가
   거른다. (3) 재조립이 중간 조각의 크기를 강제하지 않아, 짧은 중간 조각을 보내면 조립 버퍼 가운데에 **앞 메시지의 바이트**가
   남은 채 올라왔다 - 마지막이 아닌 조각은 꽉 차 있어야 한다. (4) 순번 비교가 32 비트 랩어라운드를 다루지 않아, 랩을 건너는
   역전 도착이 영원히 중복으로 보였다 - 부호 있는 거리(`SeqDistance`)로 비교한다. (5) WS 제어 프레임의 FIN 과 125 바이트
   상한(RFC6455 §5.5)을 검사하고, 스크래치를 넘는 시스템 메시지는 잘라서 해석하지 않고 끊는다.
   **보간과 헛일**: (6) 수신 시스템이 `alpha` 를 언제나 1 로 주어 보간이 꺼져 있었고, 새 스냅숏이 없어도 매 고정 스텝마다 전체를
   다시 입혔다(`m_dirty` 는 세우기만 하고 읽는 곳이 없었다) - 복제가 스냅숏 간격을 스스로 재어 계수를 정하고, 간격을 모르면
   1 이며, 새 것도 없고 보간할 것도 없으면 일하지 않는다.
   **병목**: (7) `SendDeltas` 가 클라이언트마다 전체 스냅숏을 다시 인코드했다(O(클라이언트 × 엔트리)). **§3-4 의 1.86 ms 는
   클라이언트 하나일 때의 값이다.** 이제 기준(ack 한 틱)마다 한 번만 인코드하고 같은 기준끼리 나눠 쓴다 - 진단의
   `lastDeltaEncodes` 가 그 수다. (8) `Broadcast` 가 연결마다 `Send` 를 불러 연결을 다시 찾았다(O(n²)) - 찾기와 검사를
   `SendTo` 로 나눴다.
   **엔진 쪽**: `StartServer` / `Connect` 는 이제 예산을 잡는 자리이므로 예외를 여기서 잡아 거짓으로 돌린다 - 스크립트 DLL 이
   부른 서비스 호출을 타고 예외가 경계를 넘지 않게 한다. 호스트 큐가 넘치면 `Overflow` 를 알린다(지금 예산에서는 닿지 않는 길이고,
   두 예산이 갈라지는 날을 위한 자리다).
   테스트 +7(모르는 채널·짧은 중간 조각·랩어라운드 역전·고정 슬롯·브로드캐스트·기준별 한 번 인코드·보간과 멈춤).
   뮤테이션 6/6 잡힘.
   `[열림]` 진짜 OOM 은 이 프로젝트가 고칠 수 있는 것이 아니다 - `Array` 의 기본 할당자는 실패할 때 `nullptr` 을 돌려주고
   `Reallocate` 가 그것을 그대로 쓴다(JBroCore 전체의 문제다). 예외를 잡는 것은 던지는 할당자·컨테이너를 위한 대비다.

9. `[열림]` wss(SChannel), POSIX 소켓(기존도 미검증), 네이티브 WebRTC 피어(libdatachannel 같은 스택 - 지금 네이티브 provider 는
...
- `[확정 2026-09-20]` §2.2 위치 `source/JBroNetwork/`. §2.7 시그널링 서버는 라이브러리(`Peer/Signaling`)로 섰고, 게임 호스트 실행 파일의
  옵션으로 켜는 일은 §3-9 에 남았다.
- `[닫힘]` §2.6 풀 비교 비용 - 실측 결과 dirty 비트 없이 예산 안(§3-4).
- `[열림]` §2.7 네이티브 WebRTC 피어·TURN. §3-6 브라우저 접착 검증. §3-9 전부.
