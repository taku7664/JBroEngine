...
| 2026-09-13 | 6 | GPU 읽기 경로와 스프라이트 픽셀 검증 (§12.4 닫음) | `af17285` 직전 |
| 2026-09-13 | 6 | `.jproject` 읽기, 호스트가 프로젝트에서 스크립트 경로를 받음 | `af17285` |
| 2026-09-13 | 6 | H5 이름으로 스크립트 생성·파괴 (OD3 의 지목 항목) | `a1e54eb` |
...
| 틴트 속성 오프셋 28 → 24, **읽기 경로가 생긴 뒤** | `the sprite must paint the tint the packet carried, channel for channel` |
| DLL 에 스크립트 레지스트리 바인딩 제거 | `a loaded script DLL must register into the host script registry` |

| DLL 이 내려갈 때 스크립트 표를 비우지 않음 | `real script DLL must unload and reload from a Korean path` |
| 이름으로 붙인 스크립트를 파괴에서 못 찾음 | Canvas 해체 단언 (`Canvas object teardown lost ownership state`) |
| 모르는 블록을 건너뛰지 않음 | `the legacy project shape must parse` |
| `ScriptPool` 이 참조 남은 제어 블록도 재활용 | **잡히지 않음** — §14.5 참조 |

**§12.4 의 그 변이가 이제 잡힌다.** 표에서 비어 있던 칸이 채워졌고, 새로 하나가 열렸다 —
그 하나는 테스트 구멍이 아니라 **도달 불가능한 코드**였다(§14.5). 엉뚱한 곳에서 터진 것은 없다.
...
## 14. 열려 있던 셋 (2026-09-13)

§13.10 이 사용자 확인이 필요하다고 적은 셋이다. 빡대리가 "기존 엔진 확인해보면 알잖아"라고
짚어 준 대로, **셋 다 기존 엔진에 답이 있었다.** 새로 정한 것이 아니라 그 답을 따라갔다.

### 14.1 GPU 읽기 경로 — §12.4 의 구멍을 닫았다

`IRHIDevice::ReadTexture` 가 텍스처를 읽기 버퍼로 복사하고 GPU 를 기다린 뒤 행 패딩 없이
목적지에 쓴다. **진단 경로임을 스스로 말한다** — 멈춰 서고, 프레임 안에서는 거절하고,
구현하지 않은 백엔드를 위해 기본이 `false` 다. `Renderer::ReadBackBuffer` 가 방금 제시한
프레임을 가리킨다.

테스트는 64x64 표면의 왼쪽 절반에 스프라이트 하나를 그린다. 틴트는 세 채널이 다 다르고,
제시한 뒤 픽셀을 되읽어 양쪽 절반을 본다.

**§12.4 의 m22 를 다시 걸었다 — 이제 잡힌다.** 틴트 속성 오프셋을 28 → 24 로 옮기면
`the sprite must paint the tint the packet carried, channel for channel` 이 터진다.
전에는 전 스위트가 통과했다. 변이 45개 중 45개.

### 14.2 프로젝트 파일 — `.jproject`, 기존 엔진의 형식 그대로

형식을 새로 정하지 않았다. 기존 엔진이 `.jproject`(YAML)를 쓰고 키 이름도 이미 정해져 있다.
**두 번째 형식을 만드는 것이 실수였을 것이다.**

읽는 것은 그 파일들이 실제로 쓰는 부분집합이고, 나머지는 줄 번호와 함께 거절한다 —
탭, 홀수 들여쓰기, 닫히지 않은 따옴표, 키가 요구하는 타입과 맞지 않는 값. 이 엔진이 아직
읽지 않는 키 아래의 블록은 들여쓰기로 통째로 건너뛴다.

**테스트가 실물을 읽는다.** 손으로 옮겨 적은 표본이 아니라 기존 엔진의 `Test.jproject` 다.
그리고 그것이 두 가지를 잡았다 — 옮겨 적은 표본에는 없던 `AudioBuses` 의 **맵 시퀀스**와
`ImGuiIniSettings` 의 **블록 스칼라(`|+`)** 다. 파서의 첫 두 판이 둘 다에서 죽었다.

`EngineInstance::OpenProjectFile` 이 하나를 읽어, 해석한 스크립트 경로를 §13.6 의 로더에 넘긴다.

### 14.3 리플렉션 — 이름으로 만드는 경로 (H5, Open Decision 3)

기존 엔진의 `CreateScriptFunc` 는 캔버스를 받아 직접 컴포넌트를 붙였다. **여기서는 그럴 수 없다** —
`Canvas` 는 Tier E 이고 스크립트 타깃의 include 경로에 없다(D-42). 그래서 모양을 바꿨다:
DLL 은 아무것도 할당하지 않고 **크기·정렬·제자리 생성·파괴**만 등록하고, 메모리는 호스트 풀이 잡는다.
경계를 넘는 것은 POD 와 함수 포인터뿐이다.

`ScriptPool` 은 `TObjectPool` 이 하는 일을 슬롯 크기를 실행 시간에 받아서 한다.
`Canvas::AttachScript(owner, name)` 이 그것을 거치고, 붙은 뒤로는 정적으로 붙인 컴포넌트와
완전히 같은 길을 간다 — 등록, 부착, 스케줄.

`ScriptRegistry` 도 `Local`/`Get`/`Bind` 를 갖는다(세 번째다). 로드 컨텍스트에 `Scripts` 가 붙어
ABI 는 4 다. 모듈이 내려갈 때 표를 비운다 — 그 함수 포인터들이 사라질 코드 안에 있기 때문이다.

**테스트가 진짜 DLL 로 돈다.** 호스트가 정의를 보지 못하는 타입을 DLL 이 등록하고, 호스트가
이름으로 붙이고, DLL 이 보고한 크기만큼 잡았는지 보고, 스케줄에 올라오는지 보고, 소유
오브젝트를 파괴한다. **마지막 단계가 진짜 결함을 잡았다** — 파괴가 컴포넌트 버킷만 뒤져서
이름으로 붙인 스크립트를 부수지 못했고 Canvas 해체가 단언에서 멈췄다.

**여전히 절반이다.** 프로퍼티, 인스펙터 메타데이터, 직렬화는 없다. 있는 것은 이름으로
만들고 부수는 것이며, 그것이 Open Decision 3 이 지목한 바로 그 항목이다.

### 14.4 이 라운드에서 내가 틀린 것

- **테스트의 인자 평가 순서.** `Check(Parse(...), error.message.c_str())` 에서 메시지가 먼저
  평가될 수 있어 쓰레기 문자열이 나왔다. 먼저 돌리고 나서 묻도록 고쳤다.
- **대기 루프.** MSBuild 는 노드 재사용으로 워커를 남긴다. "MSBuild 가 사라질 때까지" 기다린
  루프가 끝나지 않았다. 확인 스크립트에 `-nr:false` 를 넣었다.

### 14.5 잡히지 않은 변이 하나 — 테스트가 아니라 코드가 문제였다

`ScriptPool::Create` 가 참조 남은 제어 블록도 재활용하게 바꿨는데 전 스위트가 통과했다.
"재사용된 슬롯에서 오래된 참조가 되살아나면 안 된다"를 단언하는 테스트를 새로 붙였는데도
여전히 통과했다.

**테스트 구멍이 아니었다.** `Destroy` 가 참조가 남은 블록을 슬롯에서 이미 떼어 놓으므로,
`Create` 가 보는 non-null 블록은 언제나 참조 0 이다. 그 조건은 **도달할 수 없는 코드**였고,
실행되지 않는 코드는 틀렸는지조차 알 수 없다.

조건을 지우고 불변식을 `assert` 로 남겼다. `Destroy` 를 바꾸는 사람이 조용한 오동작 대신
단언에서 멈춘다. 새 테스트는 그대로 둔다 — 그 성질 자체는 붙잡아 둘 값어치가 있다.
