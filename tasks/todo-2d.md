...
- `[진행]` **2D 물리 이식** (D-199). 기존 엔진 물리를 이어받되 오목 폴리곤 결함(계획서 §1.2 의 여섯 원인)을 고친다. 단계는 계획서 §4:
  ...
  `[완료]` 1~3 단계 - 커널 `JBroPhysics2D`(기하·판정·월드와 솔버)가 섰고 테스트와 뮤테이션(14/15·17/19·17/17, 살아남은 것은
  동치)이 계획서 §4 에 있다. 엔진은 아직 이 커널을 쓰지 않는다 - 지금 `Physics2DSystem` 은 여전히 중력 적분뿐이다.
  `[논의]` 4 단계: 물리 시스템이 오브젝트의 `GameScript2D` 만 골라 충돌 훅을 부르는 길(계획서 §6). 스크립트 DLL ABI 를 건드린다.
...
- `[진행]` **텍스트** (D-200, [text-plan.md](./text-plan.md)). 커널 `JBroText` + 글자마다 스프라이트 인스턴스 + `TextStore`.
  ...
  저장소는 프로세스에 하나다(D-206). 다음은 §5 의 3 단계(에디터: 여러 줄 글자 칸·캔버스 뷰 선택·폰트 임포트 옵션·프로젝트 기본 폰트)다.
