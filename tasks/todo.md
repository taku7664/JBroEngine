...
- `[열림]` **네트워크의 남은 것**(D-122, [network-plan.md](./network-plan.md)). 여섯 단계(뼈대 → WS → Reliable UDP →
  ...
  브라우저 접착(§3-6)의 검증이다.
- `[진행 예정]` **오디오**(D-197, [audio-plan.md](./audio-plan.md)). 계획과 결정만 섰고 코드는 없다. 공용 쪽 단계는 계획서 §3 의
  1(믹서 뼈대와 오프라인 렌더)·2(출력과 에셋)·4(스크립트 서비스)·5(에디터)다. 2D 컴포넌트와 시스템은 todo-2d, 3D 리스너는 todo-3d.
  다음은 1 단계다.
