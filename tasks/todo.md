...
- **D-210. 텍스트 저장소는 캔버스마다가 아니라 프로세스에 하나이고(`JBroRuntime` 의 `TextStore`), `TextId` 는 코덱으로 값처럼 행동한다.**
  (2026-09-25, text-plan §5 의 2 단계. 처음 D-203, 다음 D-206 으로 적었다가 두 번 다 main 의 오디오 결정과 겹쳐 D-210 으로 옮겼다 - 커밋 `7d686a8`·`d1ee18e` 의 메시지는 옛 번호다) Updates: D-200 (1) 의 "캔버스마다 하나인 `TextStore` 를 Framework2DSystem 이 든다". 구현에서 두 사실이
