...
- `[열림]` **네트워크의 남은 것**(D-122, [network-plan.md](./network-plan.md)). 여섯 단계(뼈대 → WS → Reliable UDP →
  ...
  브라우저 접착(§3-6)의 검증이다.
- `[진행]` **오디오**(D-197·D-198·D-201, [audio-plan.md](./audio-plan.md)). 1~5 단계(믹서·출력·에셋·2D/3D 소스·스크립트 서비스·
  에디터)가 섰다. 남은 것은 계획서 §3-6(이펙트)와 §5 의 `[열림]`(버스 중첩·솔로·센드, 디스크 스트리밍, Web autoplay·장치 선택·
  핫 언플러그·포커스 정책)이다.
...
- **D-203. 텍스트 저장소는 캔버스마다가 아니라 프로세스에 하나이고(`JBroRuntime` 의 `TextStore`), `TextId` 는 코덱으로 값처럼 행동한다.**
