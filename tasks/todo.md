...
- 필드를 가진 **구조체 원소의 목록**을 그리지도 고치지도 못한다. 옮기기는 섰다(D-89 진행 중).
- bool·int·enum 원소 목록과 목록 끌어 옮기기는 **마우스로 재지 않았다**(D-86, D-89 ⑤).
...
- **D-89. 필드를 가진 구조체 원소의 목록을 그리고 고치고 옮긴다.** (진행 중, `Updates` D-86)
  ...
  **스크립트 타입과의 관계(사용자 메모, 2026-09-15)**: 스크립트는 엔진 타입만 쓰고 `Int`(64비트)·
  ...
  엔진 `Vec2` 의 이름은 §12 의 2번으로 열려 있어 여기서 바꾸지 않았다.
