...
그리고 **고른 전부에 미치는 편집**(D-83)이 들어갔다.
테스트가 `JBRO_EDITOR_SHOT` 을 주면 화면을 파일로 찍는다 - 사람이 창을 띄워 보는 것은
반복되지 않기 때문이다(§11.4).

다음에 할 만한 것 — 한 일에서 곧바로 이어지는 것들이다:
- 커맨드가 여섯이다(프로퍼티·만들기·지우기·컴포넌트 붙이기·떼기·묶음). 기존에는
  서른 개 가까이 있다 — 컴포넌트 순서, 레이어, 부모 바꾸기, 복사/붙여넣기.
  묶는 쪽(`CompoundCommand`)이 생겼으므로 "한 동작이 여러 값을 바꾼다" 는 자리는
  이제 새 커맨드 없이 선다.
