...
- `[진행 예정]` **4. 로딩은 비동기 - 로딩 관리자가 스레드를 들고 로드 태스크를 등록받는다**(D-208). 툴마다 로딩 연출을 따로 만들지 않는다.
  ...
  - **로딩 현황표**: 등록된 태스크를 목록으로 늘어놓아 사용자가 볼 수 있게 한다 - 태스크마다 이름·상태(대기·진행·완료·실패)·진행률.
    기존 엔진 `RootDockWindow.cpp` 가 태스크 목록 앞에 스피너를 그렸다(완료는 초록, 미완료는 빨강) - 구현 전에 그 화면을 먼저 읽는다.
    지금은 `Widget::LoadingSpinner` 그리기 함수만 있고, 누가 언제 무엇을 기다리는지는 각자 정한다.
  - 지켜야 할 것: 워커는 `TextureData` 같은 POD 자료만 만들고, 풀에 넣거나 `SafePtr`·`Ref<T>` 를 만지는 마무리는 메인 스레드가 한다
    (메인 스레드 전용, D-54). 그래서 태스크는 "워커에서 하는 일" 과 "메인에서 마무리하는 일" 두 조각으로 등록한다.
    아래 `[열림]` 미리 읽기(prefetch) 워커와 같은 스레드를 쓸 수 있다 - 따로 세우지 않도록 함께 정한다.
  - 정할 것: 워커 수(하나인지 풀인지), 태스크 이름을 로컬라이징 키로 받을지, 진행률을 모르는 태스크의 표시, 취소(10 번), 실패했을 때 1 번 알림으로
    이어 줄지, 현황표를 별도 패널로 둘지 상태 표시줄(8 번)에서 펼칠지, 다른 태스크가 끝나야 시작하는 의존 관계를 둘지.
