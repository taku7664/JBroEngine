...
> 엔진·에디터·빌드의 것만 적는다.
>
> **구현을 마친 항목은 지우지 않고 취소선으로 긋는다**(D-204). 끝에 언제(날짜·커밋)와 어디서(파일·함수) 고쳤는지 붙인다.
...
방향은 사용자가 정했고, 새 서비스를 세우기 전에 아래 "정할 것" 을 확인받는다(ProjectRule: 새 서비스는 사용자 확인 뒤).

- `[진행 예정]` **1. 우측 하단 알림(토스트) API.** 에디터 창 우측 하단에 알림을 쌓아 띄우고, 패널·툴·외부 에디터가 한 함수로
  부른다. 지금은 없다 - 사용자에게 알리는 길은 `MessagePopup`·`ConfirmPopup`(막는 팝업)과 `LogPanel`(D-133, 열어야 보임)뿐이다.
  정할 것: 등급(정보·경고·오류·성공), 표시 시간과 수동 닫기, 동시에 쌓이는 개수 상한, 같은 알림 합치기, 알림을 로그(`JBro/Core/Log.h`)에도
  남길지, 누르면 동작(예: 로그 창 열기)을 붙일지. 글자는 로컬라이징 키로 받는다.
- `[진행 예정]` **2. 단축키를 공용 API 로 - 이름 붙은 Action 과 사용자 재매핑.** 각 에디터는 단축키를 **Action 이름으로 등록**하고,
  ...
  은 보기만 한다. 등록 API·재매핑·저장이 없다. `Describe` 가 이미 키 이름을 ImGui 에게 물으므로 이름 표시는 그 길을 잇는다.
  정할 것: Action 이름 형식(`editor.save_canvas` 같은 점 구분 문자열인지, 등록 때 한 번만 해시해 매 프레임 문자열 비교를 피하는지),
  사용자 매핑의 저장 위치(에디터 설정 파일, YAML), 충돌한 조합의 처리, 기본값으로 되돌리기, 외부 에디터가 등록한 Action 이 빠졌을 때
  남은 사용자 매핑을 어떻게 둘지.
...
- `[진행 예정]` **4. 로딩은 비동기 - 로딩 관리자가 스레드를 들고 로드 태스크를 등록받는다**(D-208). 툴마다 로딩 연출을 따로 만들지 않는다.
  - **로딩은 무조건 비동기다.** 로딩 관리자(이름 미정) 하나가 워커 스레드를 소유하고, 패널·툴·외부 에디터는 **로드 태스크를 등록**만 한다.
    메인 스레드에서 디스크를 읽거나 디코드하며 멈추는 로딩을 새로 만들지 않는다.
  - **워커는 여러 개다.** 관리자가 워커 풀을 소유하고 수는 설정으로 정한다(기존 엔진 `TaskManagerDesc::WorkerCount` 처럼 0 이면 코어 수에서 정함).
  - **진행률은 하위 작업 수로 센다.** 태스크를 만들 때 `numSubTasks`(기본 1)를 받는다 - 기본은 `0/1`, 리소스 40 개를 읽는 태스크는 40 을 넣어 `0/40`
    에서 시작한다. **등록하는 쪽이 몇 개인지 미리 알아야 한다.** 워커는 하위 작업 하나를 끝낼 때마다 태스크의 원자 `uint32` 카운터
    `succeededSubTasks` 를 올린다. 메인 스레드는 그 값을 읽기만 해 현황표를 그린다.
    이 설계는 기존 엔진이 겪은 문제를 피한다: `CTaskGroup::AllCompletedCallback` 은 큐에 넣는 도중 빠른 태스크가 먼저 끝나면 일찍 불려서,
    `ProjectManager.cpp` 가 총 개수를 미리 세고 자기 카운터로 마지막을 판정해야 했다. 총 개수를 등록 때 받으면 그 우회가 필요 없다.
  - **결과와 콜백.** 태스크는 끝나면 결과 값을 돌려주고, 등록한 쪽의 콜백이 **메인 스레드에서** 그 결과를 받는다(기존 엔진 `EndCallback` 도
    `PostMainThreadTask` 로 메인에서 불렸다). 결과는 열거형 하나가 아니라 구조체로 한다: 상태(`Succeeded`·`Failed`·`Canceled`)와 상세 실패 사유
    (어느 하위 작업이 왜 실패했는지 - 예: 어느 파일을 못 읽었는지). 하위 작업 일부만 실패하는 경우를 위해 원자 `failedSubTasks` 도 둔다.
  - **이름**(기존 엔진 `Core/Task` 를 따르고 접두사 `C`·`E` 만 뗀다): `TaskManager`(관리자)·`TaskGroup`(패널이 등록하는 묶음, 앞서 "로드 컨텍스트"
    라 부른 것)·`Task`·`TaskState`(`Pending`·`Running`·`Completed`·`Failed`·`Canceled`)·`TaskManagerDesc`. 기존에 없던 것은 제안이다:
    결과 구조체 `TaskResult`, 묶음의 순서 모드 `TaskGroupOrder`(`Parallel`·`Sequential`), 공용 그리기 `Widget::TaskGroupProgress(group)`·
    `Widget::TaskTable()`. 기존 엔진은 프로젝트 로딩(`RenderProjectLoadingPopup`)과 빌드 진행(`RenderBuildProgressPopup`)을 **화면마다 따로**
    그렸다 - 이 항목이 없애려는 바로 그 모양이라 이름을 잇지 않는다.
  - **실패 알림**(2026-09-26 확정): 실패와 취소는 1 번 알림으로 우측 하단에 띄우고 같은 내용을 로그에도 남긴다. 성공은 기본으로 띄우지 않고,
    태스크가 원할 때만 띄운다 - 캔버스를 열 때마다 "완료" 가 쌓이면 알림이 소음이 된다. 알림을 누르면 현황표나 로그의 그 줄로 간다.
  - **순서**(2026-09-26 확정: **묶음 단위만**). 한 `TaskGroup` 안의 태스크를 **순서대로**(앞 태스크가 끝나야 다음) 돌릴지 **병렬로** 돌릴지 묶음을
    만들 때 고른다. 태스크 사이의 개별 의존은 두지 않는다 - 그것은 개발자가 묶음을 나누거나 콜백에서 다음 묶음을 등록해 조정한다.
    기존 엔진 `Core/Task` 에는 이 설정이 없었다 - `CTaskManager` 는 큐 하나를 워커들이 꺼내 가는 병렬뿐이고, `ProjectManager` 는 에셋 로드와
    스크립트 빌드를 병렬로 넣은 뒤 자기 카운터로 끝을 기다렸다. 새 엔진에는 태스크 관리자가 아직 없다.
  - **`TaskGroup` 과 현황표.** 패널·툴이 태스크 묶음을 `TaskGroup` 으로 등록하고, 관리자는 등록된 묶음을 모두 안다.
    현황표는 공용 그리기 하나로 그린다(`Widget::TaskGroupProgress(group)` 은 한 묶음, `Widget::TaskTable()` 은 모든 묶음). 태스크마다 이름·상태(대기·진행·
    완료·실패·취소)·`succeeded/num` 을 보인다. 툴 안의 로딩도 이 그리기를 불러 모양이 같아진다.
    자리 나누기(제안): 태스크를 돌리는 관리자는 UI 를 모르는 엔진 쪽에 두고(게임의 비동기 로드와 미리 읽기 워커도 쓸 수 있게), 그리기는 에디터
    위젯 계층(`JBro::Widget`)에 둔다 - 패널이 ImGui 를 직접 부르지 않는다는 규칙과 맞는다.
    기존 엔진 `RootDockWindow.cpp` 가 태스크 목록 앞에 스피너를 그렸다(완료는 초록, 미완료는 빨강) - 구현 전에 그 화면을 먼저 읽는다.
    지금 새 엔진에는 `Widget::LoadingSpinner` 그리기 함수만 있다.
  - **스레드 계약.** 워커 본문은 `SafePtr`·`OwnerPtr` 를 만들지도 복사하지도 파괴하지도 않는다. 사용자가 잠금을 걸어 쓰는 방식이 아니다:
    `SafePtr.h` 머리 주석과 ProjectRule 이 "메인 스레드 전용" 이라 하고, 제어 블록 카운트가 일부러 비원자라 람다 캡처가 워커에서 소멸하기만 해도
    카운트가 경쟁한다 - 잠금으로 감쌀 자리가 없다. 워커에는 값·raw 포인터를 넘기고, 대상 수명은 등록한 쪽이 태스크가 끝날 때까지 보장한다.
    풀에 넣거나 `Ref<T>` 를 만지는 마무리는 메인 스레드의 콜백이 한다. 구현자가 이것만 지키면 된다.
    아래 `[열림]` 미리 읽기(prefetch) 워커와 같은 풀을 쓴다 - 따로 세우지 않는다.
  - 태스크를 돌리는 쪽은 엔진과 에디터가 함께 쓰는 `TaskManager` 다 - 아래 "그 밖의 공용" 의 `JBroTask` 항목(D-209). 이 항목(4 번)은
    그 위에 서는 에디터 쪽(현황표 그리기·알림 연결)이고, `JBroTask` 가 먼저 서야 한다.
  - 정할 것: 태스크 이름을 로컬라이징 키로 받을지, 취소(10 번)를 하위 작업 사이에서 확인하는 방식, 현황표를 별도 패널로 둘지 상태 표시줄(13 번)에서
    펼칠지.

아래 5~14 는 2026-09-26 에 네 항목을 두고 더 필요한 것을 제안했고 사용자가 모두 넣기로 한 것이다.

- `[진행 예정]` **5. 외부 에디터 등록 API.** 외부 에디터가 창·메뉴 항목·단축키 Action(2 번)·설정 항목(6 번)을 한 입구로 등록한다. 1~4 번의
...
- `[진행 예정]` **8. 우클릭 메뉴 확장점.** 빈자리 메뉴는 `EditorActions` 한 벌이지만 외부 에디터가 항목을 끼워 넣을 길이 없다.
  메뉴 항목도 2 번 Action 을 가리키게 하면 이름·단축키 글자·잠긴 까닭(D-181)이 한 벌로 따라온다.
- `[진행 예정]` **9. 명령 팔레트.** 2 번 Action 목록을 이름으로 검색해 실행하고, 옆에 단축키 글자를 보인다. 잠긴 Action 은 까닭을 보인다.
- `[진행 예정]` **10. 진행 중인 일의 취소.** 4 번 로드 태스크를 현황표에서 취소한다. 워커 조각은 취소 표시를 보고 멈추고, 메인 마무리는 부르지 않는다.
- `[진행 예정]` **11. 파일·폴더 선택 대화상자.** 지금 없다. 툴마다 경로 칸을 따로 만들기 전에 공용으로 둔다. 파일 시스템은 `IPlatform` 뒤다(D-112).
- `[진행 예정]` **12. 선택 공유.** 한 에디터에서 고른 것을 다른 에디터가 알고 반응한다(예: 스프라이트 뷰어에서 고른 프레임을 인스펙터가 보임).
  지금은 캔버스 오브젝트 선택만 공유한다. 선택 대상은 포인터가 아니라 번호로 가리킨다(편집 커맨드 규칙과 같은 까닭).
- `[진행 예정]` **13. 상태 표시줄.** 진행 중인 로드 태스크 수(4 번)와 마지막 알림(1 번)이 내려앉는 공용 자리. 누르면 현황표·로그를 연다.
- `[진행 예정]` **14. 빈 상태·오류 표시 공용 위젯.** `선택한 오브젝트가 없습니다` 같은 안내를 패널마다 직접 그리지 않고 한 위젯으로 그린다.

### 그 밖의 공용

- `[진행 예정]` **`TaskManager` - 엔진과 에디터가 함께 쓰는 Tier E 커널 모듈 `JBroTask`**(D-209, 2026-09-26 사용자 확인).
  새 엔진에는 태스크 관리자가 없다. 스레드는 `AudioMixer` 의 스트리머·`WindowsAudio`·`WindowsFileWatcher` 가 각자 `std::thread` 로 든다.
  기존 엔진은 엔진 쪽 `CTaskManager`(`Engine/Core/Task`) 하나를 에디터(`ProjectManager` 프로젝트 로딩)와 엔진(`InputSystem` 진동 타이머)이 함께 썼다.
  - **자리**: 캔버스를 모르는 Tier E 커널 모듈이다(`JBroPhysics2D` 와 같은 자리 잡기). 쓰는 곳은 게임의 비동기 로드, 위 `[열림]` 미리 읽기 워커,
    에디터 로딩(에디터 공용 기반 4 번)이다. Tier S 가 아닌 까닭: 워커가 `SafePtr` 를 못 만지는 계약을 스크립트 작성자에게까지 넘기면 지키기 어렵다 -
    스크립트용 비동기가 필요해지면 그때 핸들로 감싼 표면을 따로 낸다.
  - **모양**(에디터 공용 기반 4 번에 적은 것): `TaskManager` 가 워커 풀(수는 `TaskManagerDesc`, 0 이면 코어 수에서)을 소유한다. `TaskGroup` 은
    `TaskGroupOrder`(`Parallel`·`Sequential`)를 묶음 단위로 고르고, 태스크 사이 의존은 두지 않는다. `Task` 는 `numSubTasks`(기본 1)를 받고 워커가
    원자 `succeededSubTasks`·`failedSubTasks` 를 올린다. 끝나면 `TaskResult`(상태 + 상세 실패 사유)를 **메인 스레드 콜백**으로 넘긴다
    (기존 `PostMainThreadTask`·`DrainMainThreadCallbacks` 자리). 상태는 `TaskState`(`Pending`·`Running`·`Completed`·`Failed`·`Canceled`).
  - **스레드 계약**: 워커 본문은 `SafePtr`·`OwnerPtr` 를 만들거나 복사하거나 파괴하지 않는다(값·raw 포인터만, 수명은 등록한 쪽이 보장).
    관리자 자신도 JBro 컨테이너를 워커와 공유할 때 잠금 아래에서만 만진다.
  - **웹**: 스레드가 없으므로 기존 엔진처럼(`JBRO_PLATFORM_WEB` 이면 `m_useWorkers = false`) 메인 스레드에서 프레임마다 차례로 도는 대체 경로를 둔다.
  - **옮기지 않는 스레드**: 오디오 스트리머(실시간 마감)와 파일 감시(OS 대기에서 막혀 풀의 워커를 붙잡는다)는 그대로 둔다.
  - **기존 엔진에서 고칠 것**: 그룹 완료 콜백이 큐에 넣는 도중 일찍 불리던 문제(`ProjectManager.cpp` 가 자기 카운터로 우회했다) - 총 개수를 등록 때
    받고 묶음을 "닫은" 뒤에만 완료를 판정한다. 순서 모드가 없던 것.
  - 완료 조건: 병렬 묶음이 여러 워커에서 실제로 동시에 돌고, 순서 묶음은 앞 태스크가 끝나기 전에 다음이 시작하지 않는다. 진행률이 `0/N` 에서 `N/N`
    까지 오르고 실패 수가 따로 센다. 콜백이 메인 스레드에서만 불린다(스레드 아이디로 검사). 워커 0 개(웹 경로)에서도 같은 결과가 나온다.
    종료 때 도는 태스크를 기다리고 남은 콜백을 비운다. 취소 표시를 본 태스크가 `Canceled` 로 끝난다. Tier S 모듈이 `JBroTask` 헤더를 include 하면
    컴파일이 실패한다(음성 테스트). 테스트마다 뮤테이션으로 실제로 잡는지 확인한다.
  - 정할 것: 콜백을 비우는 시점(프레임 시작 한 번인지), 메인 스레드 대체 경로가 프레임마다 쓸 시간 예산, 워커 이름(디버거에 보일 스레드 이름).
...
- `[열림]` **네트워크의 남은 것**(D-122, [network-plan.md](./network-plan.md)). 여섯 단계(뼈대 → WS → Reliable UDP →
  ...
  브라우저 접착(§3-6)의 검증이다.
- `[진행]` **오디오**(D-197·D-198·D-201·D-202·D-203, [audio-plan.md](./audio-plan.md)). 일곱 단계(믹서·출력·에셋·2D/3D 소스·
  스크립트 서비스·에디터·버스 이펙트·버스 중첩과 센드·솔로·미터·스펙트럼·보이스 필터·디스크 스트리밍·장치 고르기·핫 언플러그·
  포커스 정책·웹 자동 재생 알림)와 8 단계(버스 페이드·클릭 없는 음소거·더킹·트림, D-205), 9 단계(다시 꽂힌 장치로 되돌아가기·버스 사용자 처리기·웹 자동
  재생 실측, D-206), 10 단계(EQ·디스토션·코러스·피치 시프트·컴프레서·출력 리미터, D-210)가 섰다. 남은 것은 계획서 §5 의 `[열림]` 이다.
- `[논의]` **입력**([input-plan.md](./input-plan.md)). 게임 쪽 입력이 없어 스크립트가 키를 읽지 못한다. 2026-09-25 에 기존 엔진
  입력을 분석하고 구조를 제안했다(이벤트에서 프레임 상태, 반환값 `Block` + 장치 단위 소비, 핸들러는 스크립트 실행 순서 목록과
  함께 세움, `OnUpdate` 폴링은 막히고 남은 것만). 계획서 §5 의 질문 다섯이 정해지면 Decisions 로 옮긴다.
- `[열림]` **미리 읽기(prefetch) 워커.** 뜻: 다음 캔버스가 쓸 에셋 집합을 워커 스레드가 미리 디스크에서 읽고 디코드해 두어,
  ...
  메인 스레드가 한다(`SafePtr` 는 메인 스레드 전용). 워커는 따로 세우지 않고 위 `JBroTask` 의 풀을 쓴다(D-209).
...
- **D-211. 텍스트 저장소는 캔버스마다가 아니라 프로세스에 하나이고(`JBroRuntime` 의 `TextStore`), `TextId` 는 코덱으로 값처럼 행동한다.**
  (2026-09-25, text-plan §5 의 2 단계. 처음 D-203, 다음 D-206·D-210 으로 적었다가 세 번 다 main 의 오디오 결정과 겹쳐 D-211 로 옮겼다 - 커밋 `7d686a8`·`d1ee18e`·`8a42e35` 의 메시지는 옛 번호다) Updates: D-200 (1) 의 "캔버스마다 하나인 `TextStore` 를 Framework2DSystem 이 든다". 구현에서 두 사실이
  ...
  (차원 무관 - 3D 텍스트도 같은 것을 쓴다). D-51 은 그대로다: 스크립트는 저장소에 직접 쓰지 않고 `IText2DSystem`(호스트 코드)을 거친다.
  정한 것: (a) `TextId{index, generation}` 의 코덱은 파일·인스펙터에 글자로 적고, `FromText`·`Assign` 은 **받는 쪽의 칸에 글자를 쓴다**(칸이
  없으면 새 칸) - 복사된 컴포넌트가 한 칸을 나눠 갖지 않는다. C++ 복사로 같은 칸을 가리키게 된 둘 사이의 `Assign` 은 받는 쪽에 새 칸을 준다.
  칸은 `Text2D::OnDetached` 에서 돌려준다. (b) 캔버스 파일의 YAML 은 값을 이스케이프하지 않고 여러 줄을 읽지 않으며, 따옴표를 벗긴 **뒤에**
  `[]`·`{}` 를 빈 컨테이너로 읽는다. 그래서 코덱이 `\` `\n` `\r` `\t` `"` 와 첫 글자의 `[`·`{` 를 이스케이프하고, 앞뒤 공백·작은따옴표로
  시작·이스케이프가 든 값은 큰따옴표로 감싼다. 코어 YAML 은 고치지 않았다(모든 파일에 번진다). 인스펙터 한 줄 칸에서는 `\n` 이 줄바꿈이다.
  (c) 판번호(`GetRevision`)가 변경 감지다. 기각: Framework2DSystem 의 캔버스별 저장소(위 두 사실), 컴포넌트마다 고정 버퍼(길이 제한, D-200 에서
  이미 기각), 코어 YAML 에 따옴표·이스케이프 더하기(영향 범위).

- **D-209. 태스크 관리자는 엔진과 에디터가 함께 쓰는 Tier E 커널 모듈 `JBroTask` 다.** (2026-09-26) Updates: D-208.
  사용자가 정했다. 새 엔진에는 태스크 관리자가 없고 스레드를 쓰는 자리마다 `std::thread` 를 따로 든다. 에디터 로딩(D-208)·게임의 비동기 로드·
  미리 읽기 워커가 모두 같은 워커 풀을 부르므로, 에디터 안에 로딩 관리자를 따로 세우지 않고 엔진 쪽 모듈 하나로 둔다 - 기존 엔진도
  `CTaskManager` 하나를 에디터와 엔진이 함께 썼다. 이름은 기존 `Core/Task` 에서 접두사만 뗀다. Tier E 인 까닭은 워커가 `SafePtr` 를 못 만지는
  계약을 스크립트 작성자에게 넘기지 않기 위해서다. 웹은 워커 없이 메인 스레드에서 차례로 돈다. 오디오 스트리머와 파일 감시 스레드는 옮기지 않는다.
  D-208 의 "로딩 관리자" 는 이 `TaskManager` 이고, 에디터는 그 위에서 현황표 그리기와 알림 연결만 맡는다.

- **D-208. 에디터의 로딩은 비동기다 - 로딩 관리자가 워커 스레드를 소유하고, 로드 태스크를 등록받아 현황표로 보인다.** (2026-09-26)
  사용자가 정했다. 툴마다 로딩을 따로 만들면 연출도 멈춤도 제각각이 된다. 패널·툴·외부 에디터는 로드 태스크를 등록만 하고,
  관리자가 워커에서 돌린 뒤 메인 스레드에서 마무리한다(워커는 POD 만, `SafePtr`·풀은 메인, D-54). 사용자는 등록된 태스크를 목록으로
  늘어놓은 현황표에서 이름·상태·진행률을 본다. 워커 수·취소·실패 알림·현황표 자리는 남은 일(공용 "에디터 공용 기반" 4·10·13 번)에서 정한다.
  같은 날 사용자가 더 정했다: 워커는 여러 개다. 진행률은 등록 때 받는 `numSubTasks`(기본 1)와 워커가 올리는 원자 `succeededSubTasks` 로 센다 -
  등록하는 쪽이 개수를 미리 안다. 태스크는 결과(상태 + 상세 실패 사유)를 내고 콜백이 메인 스레드에서 받는다. 묶음 안의 순서는 순서대로·병렬 중
  고른다(기존 엔진 `Core/Task` 에는 이 설정이 없었다). 현황표는 관리자가 아는 `TaskGroup` 을 공용 그리기 하나로 보인다.
  이어서 확정: 실패·취소는 우측 하단 알림과 로그로 알리고 성공은 태스크가 원할 때만 알린다. 순서는 묶음 단위만 두고 태스크 사이 의존은 개발자가
  묶음을 나눠 조정한다. 이름은 기존 엔진 `Core/Task` 를 접두사만 떼어 잇는다(`TaskManager`·`TaskGroup`·`Task`·`TaskState`).

- **D-207. 2D 프로젝트의 스크립트는 모두 `GameScript2D` 에서 파생하고, 그것을 등록할 때 컴파일 시간에 검사한다.**
  ...
  (처음 D-203, 다음 D-206 으로 적었으나 두 번 다 같은 날 오디오 결정과 겹쳐 D-207 로 옮겼다.)
...
- **D-204. 구현을 마친 남은 일은 지우지 않고 취소선으로 긋고, 언제·어디서 고쳤는지 붙인다.** (2026-09-26)
  ...
  이미 `[완료]` 로 남은 항목은 손대지 않고, 이후에 끝나는 항목부터 적용한다. 형식은 `~~항목~~ → 완료 날짜 · 커밋 · 파일(함수)` 이다.
  `todo.md`·`todo-2d.md`·`todo-3d.md` 와 계획서의 체크리스트에 똑같이 쓴다. ProjectRule §1 에 같은 규칙을 적었다.

- **D-210. 오디오 10 단계: 버스 사슬에 EQ·디스토션·코러스·피치 시프트·컴프레서를 더하고, 출력의 잘라 내기를 리미터로 바꾼다.**
  (2026-09-26, [audio-plan.md](./audio-plan.md) §3-10, 사용자 요청 "남은 이펙터도 추가해 완전히") Updates: D-202(네 칸 고정 사슬).
  정한 것: (1) 사슬의 차례는 고역 차단 → 저역 차단 → **3 대역 EQ**(낮은 선반·가운데 봉우리 Q 1·높은 선반, RBJ) → **디스토션**(tanh,
  가득 찬 소리는 가득 찬 채로 두게 tanh(이득)으로 나눈다) → **코러스**(12 ms + 깊이만큼 흔들리는 지연, 오른쪽은 1/4 주기 늦춤) →
  **피치 시프트**(50 ms 창 두 탭·삼각 창, 빠르기는 그대로) → 메아리 → 잔향 → 원음 양 → **컴프레서**(채널 최대의 어택·릴리스
  추종, 넘친 만큼 × (1 - 1/비율)) → 사용자 처리기 → 음량·더킹. (2) 칸마다 기본값이면 건너뛴다 - 이득 0 dB·세기 0·섞는 양 0·
  반음 0·비율 1. 값은 원자 변수, 계수는 오디오 스레드가 바뀐 것을 보고 짓고, 코러스·피치의 지연선은 처음 켤 때 메인 스레드가
  잡는다(D-202 와 같다). (3) **출력 리미터**(기본 켬, 천장 0.98): 프레임의 가장 큰 채널이 천장을 넘으면 그 순간 줄이고 0.1 초에
  걸쳐 1 로 되돌린다. 넘지 않으면 고리를 건너뛴다. 그 뒤의 1 에서 자르기는 안전판으로 남는다. `Stats::lastPeak` 는 리미터 앞의
  값이라 넘친 것이 미터에 보인다. `AudioMixer::SetOutputLimiter`. (4) `.jproject` 의 새 키(`EqLowHz`·`EqLowGain`·`EqMidHz`·`EqMidGain`·
  `EqHighHz`·`EqHighGain`·`Distortion`·`DistortionMix`·`ChorusMix`·`ChorusRate`·`ChorusDepth`·`PitchShift`·`CompRatio`·`CompThreshold`·
  `CompAttack`·`CompRelease`·`CompMakeup`)는 기본값과 다를 때만 적힌다. 프로젝트 설정의 "이펙트" 는 사슬의 차례로 늘어놓는다.
  컴프레서가 줄인 양은 `GetBusGainReduction` 이다. 기각: 앞을 내다보는(lookahead) 리미터(지연이 는다 - 게임의 효과음은 늦으면
  안 된다), 보이스마다의 이 칸들(D-202 와 같은 까닭).

- **D-206. 오디오 9 단계: 다시 꽂힌 장치로 되돌아가기, 버스 사용자 처리기, 웹 자동 재생의 실측, 실제 스피커 시험은 켤 때만.**
  (2026-09-26, [audio-plan.md](./audio-plan.md) §3-9) Updates: D-203(9)·(11), D-202 `[열림]` 사용자 이펙트.
  정한 것: (1) **장치 알림**: `IPlatform::TakeAudioDevicesChanged`(기본 거짓). Windows 는 `IMMNotificationClient` 를 **제 스레드의
  멀티스레드 아파트**에서 등록한다 - 메인 스레드의 COM 방식(파일 대화상자의 STA)과 섞이지 않는다. 알림은 원자 표지 하나다.
  호스트는 고른 장치가 없어 기본으로 떨어져 있을 때만 이 표지를 보고, 서면 고른 장치를 다시 연다(옛 장치를 멈춘 뒤 새 것을 켠다).
  목록을 매 프레임 읽지 않는다. (2) **버스 사용자 처리기**: `AudioMixer::SetBusProcessor(버스, 콜백, user)` - 사슬의 끝(잔향 뒤,
  음량 앞)에서 오디오 스레드가 부른다. 떼기(또는 바꾸기)는 오디오 스레드가 그 처리기를 부르는 중이면 그 한 번이 끝날 때까지
  기다린다(`inProcessor` 를 먼저 세우고 읽는 순서, seq_cst). 그래서 **돌아온 뒤에는 옛 처리기가 불리지 않고** 그 코드를 곧 내려도 된다.
  **스크립트 DLL 에는 열지 않는다**: 핫 리로드가 DLL 을 내리는 동안 오디오 스레드가 부를 수 있고, 스크립트 코드가 오디오 스레드의
  규칙(할당·잠금 금지)을 지킨다고 믿을 수 없다 - 엔진·호스트 코드의 확장점이다. (3) **웹 자동 재생 실측**: 엔진의
  `MiniaudioAudioOutput` 을 Emscripten(emsdk, `C:\emsdk`)으로 빌드해 브라우저에서 열었다. 자동 재생을 허락하는 브라우저는
  `unlocked` 알림 없이 곧바로 당겼고, 알림만 기다리던 `IsWaitingForUserGesture` 가 소리가 나는 동안에도 참으로 남았다(96,256
  프레임을 당긴 뒤에도 참). 고친 것: 장치가 한 번이라도 당기면 거짓이다. 막는 브라우저는 입력 전에 당기지 않으므로 그때는 참이다.
  누름 한 번에 `unlocked` 가 와서 거짓이 되는 것도 보았다. (4) **실제 스피커를 여는 시험은 `JBRO_AUDIO_DEVICE_TEST=1` 일 때만
...
- **D-205. 오디오 8 단계: 버스 음량은 이펙트 노드 끝의 램프다 - 페이드·클릭 없는 음소거·더킹. 클립마다 트림.**
  ...
  까닭: 그룹 음량(`ma_sound_group_set_volume`)은 곧바로 바뀌어 음량·음소거·솔로를 바꾸는 순간 딸깍 소리가 났고, 페이드를 걸
  자리가 없었다. 정한 것: (1) 버스 음량·음소거·솔로의 결과는 버스 이펙트 노드의 **끝**에서 샘플마다 곧게 옮겨 가는 이득이다
  (기본 10 ms). 그래서 음량은 이펙트 뒤에 걸린다 - 음소거하면 메아리·잔향의 꼬리도 멎고, 센드는 음량 뒤의 소리다. 그룹 음량은 1 로 둔다.
  (2) `SetBusVolume(버스, 음량, 페이드 초)`, 스크립트 `AudioService::FadeBusVolume`. (3) **더킹**: 버스가 다른 버스(트리거) 노드의
  지난 블록 봉우리를 읽어 0.01 을 넘으면 `DuckAmount` 만큼 20 ms 에 줄고 `DuckRelease` 초에 돌아온다. 버스 노드는 믹서의 고정
  배열에 있어 트리거 버스를 내려도 읽는 메모리는 산다. `.jproject` 의 `DuckBy:`·`DuckAmount:`·`DuckRelease:`(기본 0.3 은 적지 않음).
  (4) **트림**: `Audio.ImportOptions.gain`(0..4)이 보이스 음량에 곱해진다 - 녹음 크기는 에셋에서, 연출은 컴포넌트에서.
  기각: 스냅숏 에셋(버스 몇 개의 `FadeBusVolume` 이면 된다), `PlayOneShot` 이 핸들을 돌려주기(D-197 - 멈출 소리는 `AudioSource`
  로 둔다), 재생 마커 콜백(스크립트가 `GetTime` 을 본다), 자동 라우드니스 정규화(LUFS 를 재어 바꾸면 파일마다 결과가 달라진다 - 트림을 손으로 둔다).

- **D-203. 오디오 7 단계: 버스 중첩·센드·솔로·미터, 보이스 필터, 디스크 스트리밍, 장치 고르기와 되살리기, 포커스 정책.**
  (2026-09-26, [audio-plan.md](./audio-plan.md) §3-7) Updates: D-197(§2.2 "디스크 스트리밍은 `[열림]`", §2.4 "버스는 Master 아래 한 층"),
  D-202(`[열림]` 보이스 단위 이펙트 → 필터 두 칸). 정한 것:
  (1) **버스 부모**: 목록에서 앞에 있는 버스만 부모가 된다(파일의 차례가 곧 만드는 차례다). 그룹 → 이펙트 → 부모 그룹. `Parent:`.
  (2) **센드**: 버스 이펙트 노드의 둘째 출력(음량·이펙트 뒤)이다. 양은 miniaudio 출력 버스 음량(원자)이다. 부모·센드를 따라
  보내는 버스에 되돌아 닿는 센드는 거절한다(그래프가 돌면 오디오 스레드가 멈춘다). `Send:`·`SendLevel:`. 받는 버스는 새 칸
  `Dry`(원음 양, 기본 1)를 0 으로 두면 젖은 소리만 낸다 - 공용 잔향 버스.
  (3) **솔로**: 저장하지 않는 믹싱 상태다. 솔로 버스·그 자식·그것들이 센드로 보내는 버스·그 모두의 조상만 들리고, 버스의 음량·
  음소거 값은 바뀌지 않는다. 버스를 다시 세워도 이름으로 되살린다. 에디터 통계 창의 버스마다 칸이다.
  (4) **미터·스펙트럼**: 버스 이펙트 노드가 블록마다 최대 절댓값을 원자로 남긴다. `Render` 가 최근 4096 샘플(채널 평균)을 링에
  쓰고 메인 스레드가 2048 점 FFT(한 창)로 30 Hz~나이퀴스트를 로그 간격 칸으로 나눈다(-72..0 dB → 0..1). 통계 창에 선다.
  (5) **보이스 필터**: 보이스마다 초기화 때 만든 필터 노드(저역·고역 통과)를 켤 때만 소리 → 필터 → 버스로 잇는다 - 끈 보이스는
  비용이 없다. `AudioSource.lowPass`·`highPass`(재생 중에 바뀐다). 기각: 보이스마다 메아리·잔향(DSP 가 보이스 수만큼, D-202 와 같다).
  (6) **출력 이득**: `SetOutputGain(g, 초)` 가 장치로 가기 직전에 곧게 옮겨 간다(클릭 없음). 포커스 정책이 쓴다.
  (7) **디스크 스트리밍**: `AudioImportMode::StreamFromDisk`. 에셋은 경로와 헤더의 형식만 든다. 믹서의 스트리머 스레드 하나가
  자리(기본 8)마다 링(기본 1 초)을 채우고 오디오 스레드는 링만 읽는다 - 원자 계수기 둘의 한 명씩 큐다. 위치 옮기기는 "여기부터
  새 소리" 번호(에폭)로, 되풀이는 스트리머가 파일을 처음으로 돌려서 한다. 파일은 `IPlatform::OpenFileStream`(UTF-8, 공유 읽기)
  으로만 열고 miniaudio VFS 로 디코더에 넘긴다 - D-112 의 "파일은 플랫폼이 연다" 를 지킨다. 링은 그 자리를 처음 쓸 때 메인
  스레드가 한 번 잡는다. 기각: miniaudio resource manager(잡 스레드·자체 할당을 들여온다, `MA_NO_RESOURCE_MANAGER`), 오디오
  스레드에서 파일 읽기. 웹은 파일 스트림이 없으므로 `Streaming` 을 쓴다.
  (8) **장치 고르기**: `IPlatform::EnumerateAudioOutputs`·`AudioOutputDesc::deviceName`. 없는 이름이면 플랫폼은 열지 않고 호스트가
  기본 장치로 연다(고른 이름은 남긴다). 장치를 바꿔도 믹서는 그대로이고 믹서의 형식으로 다시 연다 - miniaudio 가 바꾼다.
  `.jproject` 의 `AudioOutputDevice`(editorLocale 과 같은 까닭으로 프로젝트에 둔다), 스크립트 `AudioService::SetOutputDevice`.
  (9) **핫 언플러그**: 우리가 멈추지 않았는데 온 miniaudio `stopped` 알림이 `IAudioOutput::IsLost` 다. 호스트가 다음 프레임에
  닫고 다시 연다(고른 장치 → 기본), 못 열면 2 초마다 다시 시도한다. 기본 장치는 WASAPI 가 스스로 따라가고(`rerouted`) 이름만 새로 읽는다.
  (10) **포커스 정책**: `AudioMuteWhenUnfocused`(기본 거짓). 호스트가 창의 FocusLost/Gained 사건을 오디오 시스템에 알리고 0.2 초에
  걸쳐 줄이고 되돌린다. 스크립트 `SetMuteWhenUnfocused`. 오디오 시스템을 내리면 출력 이득을 1 로 돌린다(믹서는 프로세스 수명).
  (11) **웹 자동 재생**: miniaudio 의 Web Audio 백엔드가 첫 `touchend`·`click`·`keydown` 에서 스스로 푼다(원문 `miniaudio.unlock`).
  엔진은 `IsWaitingForUserGesture`(스크립트 서비스까지)로 알린다 - 게임은 "눌러서 시작" 을 띄운다. 웹 빌드가 스텁이라 실측하지 못했다.
  `[열림]` 고른 장치가 다시 꽂히면 되돌아가기, 웹 실측, 사용자 이펙트 노드.

- **D-202. 오디오 이펙트는 이펙트 에셋(`.jfx`)이 아니라 버스마다 고정된 사슬이다 - 저역 차단·고역 차단·메아리·잔향.**
  (2026-09-25, [audio-plan.md](./audio-plan.md) §3-6) Updates: D-197 계획서 §3-6("`.jfx` 이펙트 에셋과 보이스·버스 이펙트 체인").
  까닭: 게임이 실제로 쓰는 것은 "일시 정지 화면에서 배경음을 먹먹하게", "동굴의 잔향", "무전기 소리" 이고 전부 **버스 단위**다.
  `.jfx` 에셋은 새 에셋 타입·로더·임포트 화면·보이스마다의 노드 사슬을 부르고, 보이스마다 이펙트를 두면 보이스 수만큼 DSP 가
  곱해진다(기존 엔진 단계 6 이 "보이스별 중복 DSP 를 줄인다" 를 남은 일로 적은 자리). 정한 것: (1) 버스(Master 포함)마다 miniaudio
  사용자 노드 하나(`BusEffectNode`)가 그룹과 부모 사이에 선다. 차례는 고역 차단 → 저역 차단 → 메아리 → 잔향이고, 각 칸은 0 이면
  건너뛴다. (2) 값은 원자 변수이고 오디오 스레드가 처리 앞에 읽는다. 필터 계수는 오디오 스레드가 목표가 바뀐 것을 보고 다시
  짓는다(RBJ 2 차, miniaudio 필터의 `reinit` 은 스레드 안전하지 않다). (3) 메아리(최대 2 초)·잔향(Freeverb 짜임, 기존 엔진과 같다)
  버퍼는 그 칸을 **처음 켤 때 메인 스레드가 한 번** 잡아 원자 포인터로 넘긴다 - 그 뒤로는 켜고 끄고 고쳐도 할당이 없다.
  (4) 노드는 입력이 없어도 돈다(`MA_NODE_FLAG_CONTINUOUS_PROCESSING | ALLOW_NULL_INPUT`) - 소리가 멎은 뒤에도 꼬리가 운다.
  (5) `.jproject` 버스 항목에 `LowPass:`·`HighPass:`·`EchoDelay:`·`EchoFeedback:`·`EchoMix:`·`ReverbRoom:`·`ReverbDamping:`·
  `ReverbMix:` 를 **기본값과 다른 것만** 적는다 - 이펙트를 쓰지 않는 파일은 바이트 하나 바뀌지 않는다. (6) 스크립트는
  `AudioService::SetBusEffects`·`GetBusEffects`·`SetBusLowPass`, 에디터는 프로젝트 설정의 버스마다 접는 "이펙트" 마디다.
  기각: `.jfx` 에셋(위의 까닭), 보이스마다 사슬(DSP 가 보이스 수만큼), miniaudio 의 `ma_lpf_node` 등 내장 노드(파라미터를 바꾸는
  `reinit` 이 스레드 안전하지 않고 메아리·잔향을 따로 이어야 한다). `[열림]` 보이스 단위 이펙트, 센드/리턴 버스, 사용자 이펙트.

- **D-201. 오디오의 2~5 단계가 섰다 - 장치는 플랫폼이 주고, 에셋은 자료를 풀기 전에 알리고, 소스는 차원 무관한 상태 기계 하나를
  두 차원 시스템이 위치만 채워 부르며, 버스는 이름으로 적힌다.** (2026-09-25, [audio-plan.md](./audio-plan.md) §3-2~5)
  Updates: D-197·D-198(구현 세부). 정한 것: (1) `IPlatform::CreateAudioOutput` → `IAudioOutput`(miniaudio `ma_device`). `EngineConfig`
  는 믹서(`audioEnabled`, 기본 참)와 장치(`audioDeviceEnabled`, 게임 호스트·에디터만 참)를 따로 켠다 - 테스트는 장치 없이 믹서를
  당긴다. 내릴 때 장치 정지가 먼저다. (2) `AudioData` 는 CPU 자료이고 믹서가 빌린다. 그래서 `AssetSystem` 은 오디오 자료를 풀거나
  바꾸기 **직전에** `AudioReleaseCallback` 을 부르고, `System::AudioSystem` 이 그 클립의 보이스를 멈추고 등록을 내린다. 재로드는
  새 자료를 다 읽은 뒤에만 알린다. (3) 소스의 상태 기계(`playOnStart` 1 회·끄고 켜면 재무장·클립 교체·끝남과 클립 없음 구분·스크립트
  정지)는 `System::AudioSystem::UpdateSource` 한 곳이고, `Audio2DSystem`·`Audio3DSystem` 은 리스너와 위치만 채운다. 리스너가 없으면
  게임 카메라 자리에서 듣는다. (4) **2D 의 가까운 소리가 한쪽 귀로 꺾이지 않게** 소스를 리스너 앞 `panDistance` 깊이에 두고 거리
  감쇠를 같은 깊이만큼 넓힌다 - miniaudio 의 pan 은 원자가 아니라 재생 중에 쓸 수 없어서 공간화의 각도로 푼다. (5) 버스는
  `AudioBusName { NameId }` 이다. 메모리는 정수이고 파일·인스펙터는 이름 글자다(코덱). 스크립트가 글자로 주면 해시만 계산한다.
  (6) `.jproject` `AudioBuses` 는 기존 키 그대로이고, 부동소수는 **가장 짧게 왕복하는 글자**로 적는다(`%.9g` 는 `0.8` 을 바꾼다 -
  D-189 위반). 새 프로젝트는 `Music`·`SFX` 로 시작하고 버스 블록은 파일 끝에 붙는다(첫 줄은 `Version`). (7) 에디터: 인스펙터의
  `clipId` 는 오디오 에셋 목록(별명 `clip` → Audio), `bus` 는 프로젝트 버스 목록(없는 이름은 회색으로 남기고 Master 로 운다고
  알림), 오디오 에셋은 형식·길이·파형(누르면 그 자리부터)·재생/정지·반복 재생·임포트 옵션을 보인다. 다른 것을 고르면 미리 듣기가
  멈춘다. 프로젝트 설정에 오디오 갈래(버스 이름·음량·추가·삭제)가 섰다.
  기각: 2D 에서 pan 을 직접 쓰기(재생 중 경쟁), 버스를 번호로 적기(목록 순서를 바꾸면 모든 캔버스가 다른 버스로 간다), 버스를
  `String` 필드로(§10.4).

- **D-200. 2D 텍스트는 캔버스를 모르는 커널 `JBroText` 가 레이아웃하고, 글자마다 스프라이트 인스턴스로 그리며, 글자는 컴포넌트 밖 `TextStore` 에 둔다.**
  (2026-09-25, [text-plan.md](./text-plan.md) §6, 사용자 확인 "전부 권고대로 해") 기존 엔진 `Text2D` 를 이어받되 그쪽의 결함(계획서 §1.2:
  HarfBuzz 를 글자 묶음마다 불러 커닝·합자가 한 번도 적용되지 않음, 글자 단위 줄바꿈, 텍스트가 바뀔 때마다 GPU 버퍼 생성, 매 프레임
  문자열 전체 해시, 컴포넌트 주소 캐시 키, 렌더 시스템의 경계 되쓰기, 화면 픽셀 외곽선 폭과 SDF 퍼짐 초과 시 사각형)을 고친다.
  정한 것: (1) 컴포넌트는 `TextId`(번호 + 세대)만 들고 UTF-8 은 캔버스마다 하나인 호스트 소유 `TextStore` 가 든다 - D-51(컴포넌트에
  `String` 금지)에 예외를 두지 않는다. 스크립트는 `Text2DService::SetText(ref, const char*, length)` 로 바꾸고, 저장소 세대가 변경 감지다.
  (2) 1 판의 폰트 라이브러리는 stb_truetype(`ThirdParty/stb/stb_truetype.h`, ImGui 의 `imstb_` 사본은 쓰지 않는다). 커널 표면은 라이브러리와
  무관하게 닫는다. (3) 새로 두는 것: Tier E 커널 모듈 `JBroText`(face·커닝·줄바꿈·레이아웃·CPU 아틀라스, `JBroCore` 만 본다), 컴포넌트
  `Text2D`, 서비스 `Text2DService`, 에셋 타입 `Font`, `TextStore`. `Text2DSystem`(Framework2DSystem)은 어댑터이고 경계는 `InstanceId` 키 캐시가
  들며 컴포넌트에 되쓰지 않는다. (4) 글자 하나가 `SpriteRenderItem` 하나다. 1 판은 `Bitmap`(RGBA8 `(255,255,255,커버리지)`)이라 지금 스프라이트
  셰이더로 그려지고 렌더러를 고치지 않는다. `Sdf` 와 외곽선(폭은 글자 픽셀, 퍼짐으로 자름)은 4 단계. 렌더 패스 그래프는 선행 조건이 아니다.
  (5) `wrapMode` 기본은 `Word`(한글도 어절), `Character` 를 고를 수 있다. (6) 폰트 패밀리·`FontStyle`·패밀리 안 폴백은 1 판에서 뺀다 -
  `Font` 에셋 하나 = face 하나, 폴백은 프로젝트의 순서 있는 폰트 목록 하나. 기각: FreeType + HarfBuzz 로 시작(범위가 한글·라틴이고 기존도
  HarfBuzz 의 값어치를 쓰지 못했다), 텍스트마다 메시(인스턴싱 렌더러에서 GPU 할당만 는다), 텍스트 전용 제출 목록(같은 레이어 안에서
  스프라이트와 섞어 정렬할 수 없다), 컴포넌트 안 고정 버퍼(길이 제한).

- **D-199. 2D 물리는 캔버스를 모르는 커널 모듈 `JBroPhysics2D` 에 두고, 오목 폴리곤은 볼록 조각을 자식 도형으로 다룬다.**
  (2026-09-25, [physics-plan.md](./physics-plan.md) §5, 사용자 확인) 기존 엔진 물리를 이어받되 그쪽의 오목 폴리곤 결함 여섯(계획서
  ...
  (6) 질의 확장(`RaycastAll`·`OverlapPoint`·`OverlapCircle`·`CircleCast`·`BoxCast`·레이어 마스크)은 **후순위지만 구현한다**
  (4 단계 뒤의 별도 단계). 기각: `JBroFramework2DSystem` 안에 두기(커널을 캔버스 없이 잴 수 없다), 통짜 오목 판정 고쳐 쓰기.
...
- **D-197. 오디오는 믹서를 직접 쓰고 miniaudio 는 부품만 가져다 쓴다. 보이스·버스는 번호로만 나가고, 스레드 사이에는 POD 명령만
  ...
  **(1)·(3) 은 D-198 이 고쳤다** - 믹서는 내부 `ma_engine` 이고 명령 링은 없다.
  ...
  경로는 파일 경로 + 전체 디코드 하나뿐이었다(계획서 §1). 정한 것: (1) 믹싱은 우리 `AudioMixer` 가 한다. `ma_engine`·노드 그래프·
  리소스 매니저는 쓰지 않고 `ma_device`·`ma_decoder`·`ma_linear_resampler`·`ma_spatializer`·필터만 쓴다. 그래서 장치 없이
  `Render` 를 당겨 **결정적인 오프라인 테스트**가 된다. (2) 보이스·버스·클립은 믹서가 소유하는 고정 풀이고 밖에는 index+generation
  핸들만 나간다 - 호출자가 쥐는 오디오 객체가 없으므로 수명 추적이 필요 없다. (3) 메인 → 오디오 명령 링, 오디오 → 메인 상태 링(SPSC,
  POD). 오디오 스레드는 할당·잠금·파일 I/O·엔진 객체 접근을 하지 않고, 클립 메모리는 오디오 스레드가 놓았다고 알린 뒤 메인이 푼다.
  (4) 출력 장치는 `IPlatform::CreateAudioOutput` 이 준다(D-122 의 소켓과 같은 규약, 기본 null). 장치는 프로세스에 하나이고 에디터
  미리 듣기는 같은 믹서의 전용 버스다. (5) 재생 컴포넌트는 차원 무관 `Component::AudioSource` 로 새 Tier S 모듈(`JBroAudioTypes`,
  `Service::AudioService` 도 여기)에 한 번 두고, 리스너는 `Component::AudioListener2D`(뒤에 3D)로 각 Framework 에 둔다. 위치는 각 차원의
  시스템(`Audio2DSystem`)이 자기 Transform 에서 채운다. 버스는 `.jproject` `AudioBuses`(기존 키)이고 컴포넌트는 `NameId` 로 가리킨다.
  (6) 스트리밍은 압축 바이트를 메모리에 두고 재생하며 디코드한다 - 파일은 `IPlatform` 으로만 연다(§2). (7) 스크립트는 값 서비스만
  보고 D-37 확장 블록으로 받는다 - 기존 `Script.Audio`(장치 직접 노출)는 §5 위반이었다.
  기각: `ma_engine` 래핑(기존 방식 - 위 문제를 다시 다뤄야 한다), 소스를 차원별로 둘로(`AudioSource2D`/`3D` - 자료에 차원 의미가 없다),
  공용 소스를 `JBroRuntime` 에(런타임 뼈대에 기능 도메인이 섞인다). 같은 날 정한 것: (8) 오디오 에셋의 임포트 옵션은 디코드 방식 `Mode` 하나다. 볼륨·루프·거리·버스 같은 재생 파라미터는
  컴포넌트에만 둔다 - 기존 엔진은 둘 다에 같은 이름을 두어 우선순위를 끝내 정하지 못했다. (9) 모듈 이름은 `JBroAssetTypes`·`JBroAsset`
  관례대로 `JBroAudioTypes`(Tier S)·`JBroAudio`(Tier E)이고 어댑터 모듈은 따로 두지 않는다. (10) 보이스 상한 기본값은 64(기존과 같다).
  (11) 기존 캔버스·메타의 오디오 자료는 이식하지 않는다 - 이 엔진으로 만든 오디오 콘텐츠가 없다.
  **아직 코드는 없다.** 단계는 계획서 §3(믹서 뼈대와 오프라인 렌더 → 출력과 에셋 → 2D 컴포넌트와 시스템 → 스크립트 서비스 → 에디터 →
  이펙트와 3D).
