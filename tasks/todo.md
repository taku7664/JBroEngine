...
이 절은 에디터 작업의 현재 상태다. 위의 2026-09-12 스냅샷은 그 시점 기록이고 에디터를 담지
않는다. 근거는 D-60, D-63~D-74.
...
- **되돌리기**(D-71): 드래그 하나가 되돌리기 하나. 커맨드는 `SetProperty` / `CreateObject` /
  `DeleteObject` 셋. 오브젝트는 포인터가 아니라 에디터 번호로 가리킨다(D-72).
...
- 이번 구간(패널·인스펙터·되돌리기·오브젝트 커맨드)에 **뮤테이션을 아직 안 돌렸다.**
  도구는 `scratchpad/mutate.py`(목록 파일 + 범위). 한 번에 서너 개씩 돌린다.
- 커맨드가 셋뿐이다. 기존 엔진에는 서른 개 가까이 있다 — 컴포넌트 추가·제거·순서,
  레이어, 다중 선택 트랜스폼, 복사/붙여넣기.
- **다중 선택이 없다.** 기존은 선택된 전부에 같은 델타를 적용해 되돌리기 하나로 묶는다.
- 게임 뷰 **매 프레임 opt-in**(패널이 안 보이면 렌더를 멈춘다). 기존 엔진의 방식이고
  D-63 에 적어 두었다.
- 팝업(기존 `ImPopupDesc` — 핸들, Id 중복 방지, 모달), FontAwesome 아이콘.
- `Save Canvas` 메뉴가 회색이다. 경로를 받을 곳이 없다.
- 배열·표는 인스펙터에서 개수만 보인다. `ArrayOps` / `TableOps` 구현이 아직 없다.
