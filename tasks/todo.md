...
- **D-146. 에디터 세션이 프로젝트 파일에 남는다. 에디터 언어를 고를 수 있다.**
  ...
  화면의 절반이 키로 나온다.
  `[열림]` **창 배치**(ImGui `.ini`)는 아직 남지 않는다. 기존은 그것도 프로젝트에 적었다.
  우리는 첫 프레임에 `DockBuilder` 로 기본 배치를 잡는데(D-134), 적힌 배치를 읽으면 둘이
  같은 프레임에 겹친다 - 어느 쪽이 이기는지 정하지 않고 넣으면 배치가 프레임마다 흔들린다.
