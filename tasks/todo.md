...
- `[진행 예정]` **오디오**(D-197, [audio-plan.md](./audio-plan.md)). 계획과 결정만 섰고 코드는 없다. 공용 쪽 단계는 계획서 §3 의
  ...
  확인 전인 것: 임포트 옵션에서 재생 파라미터를 빼는 안(계획서 §2.2 `[제안]`).
...
- **D-197. 오디오는 믹서를 직접 쓰고 miniaudio 는 부품만 가져다 쓴다. 보이스·버스는 번호로만 나가고, 스레드 사이에는 POD 명령만
  ...
  미리 듣기는 같은 믹서의 전용 버스다. (5) 재생 컴포넌트는 차원 무관 `Component::AudioSource` 로 새 Tier S 모듈(가칭 `JBroAudioTypes`,
  ...
  공용 소스를 `JBroRuntime` 에(런타임 뼈대에 기능 도메인이 섞인다). `[제안]` 임포트 옵션에서 재생 파라미터를 빼는 것은 확인 전이다(계획서 §2.2).
