...
- **구조체 원소의 목록**이 섰다(D-89 ①~④): 접기 마디, 원소 안 필드 편집이 고른 전부에 한
  되돌리기, 표를 끊고 줄 전체. 남은 ⑤ - 끌어 옮기기와 bool·int·enum 원소 목록을
  **마우스로 재지 않았다**(D-86, D-89).
...
- **D-89. 필드를 가진 구조체 원소의 목록을 그리고 고치고 옮긴다.** (진행 중, `Updates` D-86)
  ...
  **남은 것(⑤ 와 그 뒤)**: 끌어 옮기기와 bool·int·enum 원소 목록을 마우스로 재기. 펼침 상태는 행 번호에
  붙어 옮기면 자리에 남는다. 원소 안 `NoSerialize` 필드의 값은 목록을 편집하거나 되돌릴 때 기본값이 된다
  (스크린샷에서 `echo` 5 → 0). 행 배경과 끌기 자리는 첫 줄만 덮는다. 인스펙터가 아직 `ImGui::` 를 직접
...
- **D-91. 한국어 문구는 번역체로 쓰지 않는다.** `ProjectRule.md` §11.2 에 MUST 로 넣었다(`Updates` D-80).
  ...
  **[열림] 컴포넌트 이름과 필드 이름이 영어 그대로다.** 인스펙터 머리는 타입 이름(`Transform2D`), 필드 라벨은
  필드 이름(`position`)을 보인다 - 키가 아니라 리플렉션에서 나오는 글자라 이 표에 없다. 기존 엔진은
  `editor.component.<타입>` 키(`트랜스폼 2D`)로 한국어화했다. 키 체계(타입·필드마다 키를 지을지, `Name()`
  어트리뷰트로 받을지)를 정해야 해서 사용자 확인이 필요하다.
