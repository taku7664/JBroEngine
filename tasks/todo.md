...
- **구조체 원소의 목록**이 섰다(D-89 ①~⑤): 접기 마디, 원소 안 필드 편집이 고른 전부에 한
  ...
  남은 것은 D-89 끝의 잔여 결함(펼침 상태가 자리에 남는 것, 행 배경이 첫 줄만 덮는 것,
  인스펙터의 `ImGui::` 직접 호출)이다.
...
- **D-89. 필드를 가진 구조체 원소의 목록을 그리고 고치고 옮긴다.** (①~⑤ 완료, `Updates` D-86)
  ...
  **남은 것(그 뒤)**: 펼침 상태는 행 번호에 붙어 옮기면 자리에 남는다. 원소 안 `NoSerialize` 필드의 값은 목록을 편집하거나 되돌릴 때 기본값이 된다
