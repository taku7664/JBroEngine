...
- `[진행 예정]` **4. 로딩은 비동기 - 로딩 관리자가 스레드를 들고 로드 태스크를 등록받는다**(D-208). 툴마다 로딩 연출을 따로 만들지 않는다.
  ...
  - `[논의]` **`TaskManager` 를 엔진과 에디터가 함께 쓰는 모듈로 세울지.** 새 엔진에는 태스크 관리자가 없고(스레드는 `AudioMixer` 의 스트리머,
    `WindowsAudio`, `WindowsFileWatcher` 가 각자 `std::thread` 로 든다), 기존 엔진도 엔진 쪽 `CTaskManager` 하나를 에디터(`ProjectManager`)와
    엔진(`InputSystem` 진동 타이머)이 함께 썼다. 제안: 캔버스를 모르는 **Tier E 커널 모듈 `JBroTask`** 로 세운다(`JBroPhysics2D`·`JBroText` 와 같은
    자리 잡기). 엔진은 게임의 비동기 로드와 미리 읽기 워커에, 에디터는 로딩 현황표에 쓴다. Tier S 가 아닌 까닭: 워커가 `SafePtr` 를 못 만지는 계약을
    스크립트 작성자에게까지 넘기면 지키기 어렵다 - 스크립트용 비동기가 필요해지면 그때 핸들로 감싼 표면을 따로 낸다.
    웹은 스레드가 없으므로 기존 엔진처럼(`JBRO_PLATFORM_WEB` 이면 `m_useWorkers = false`) 메인 스레드에서 차례로 도는 대체 경로를 둔다.
    오디오 스트리머와 파일 감시 스레드는 **옮기지 않는다** - 하나는 실시간 마감이 있고 하나는 OS 대기에서 막혀 있어 풀의 워커를 붙잡는다.
    새 모듈이므로 사용자 확인 뒤 진행한다.
