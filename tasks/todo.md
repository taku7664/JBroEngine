...
**뮤테이션으로 잰다.** 도구는 `tools/mutate.py`(저장소 안에 있다). 에디터 구간에 지금까지 100개 남짓 돌렸고, 살아남은 것은
...
- **배열 원소 편집이 커맨드가 아니다** - 컨테이너 전체를 글자로 떠서 되돌리는 방향으로
  구현 중이다(D-86). 표도 스냅샷·파일에 들어간다.
...
- **D-86. 컨테이너 편집은 컨테이너 전체를 글자로 떠서 되돌린다.** D-82 의 "원소 번호를
  길에 담는다" 방향을 `Updates` 한다. **확정한 방향이고 구현 중이다.**
  **길은 처음 만나는 컨테이너에서 멈추고, 그 아래는 컨테이너의 글자가 담는다.** 원소 값·
  추가·삭제·재정렬이 모두 "컨테이너 전체의 전과 후" 를 든 `SetPropertyCommand` 하나다.
  기존 `CSetComponentSerializedPropertyCommand` 와 같다. 여럿 고르면 목록 위젯이 알려 주는
  연산(원소 i 변경·추가·i 삭제·이동)을 대상마다 다시 적용하고 그 결과를 뜬다(D-83).
  **값을 YAML 로 쓰고 읽는 걸음을 `CanvasFile.cpp` 에서 JBroCore 로 옮겨** 캔버스 파일·
  스냅샷·커맨드가 함께 쓴다. 배열과 **표도** 그 걸음에 넣는다(표 편집 UI 는 여전히 개수만).
  기각: **길에 원소 번호 담기** - 길이 "필드인가 원소인가" 를 들어야 하고 깊이 4 가 모자라며,
  원소 추가·삭제·이동 커맨드 셋과 원소 스냅샷이 따로 필요하다. 대가는 되돌리기 기록에
  배열 전체가 들어가고 드래그 중 프레임마다 전체를 글자로 두 번 바꾸는 것이다.
  같이 드러난 구멍: 컨테이너는 스냅샷이 **조용히 건너뛴다**(D-76 위반). 스냅샷은 잎사귀를
  512바이트 고정 버퍼로 읽고 **읽지 못한 값을 조용히 빼는데**, 지금은 닿는 값이 없다 -
  `String` 을 컴포넌트 필드에 두지 않으므로(§10.4). 컨테이너 전체를 글자로 뜨는 순간
  원소 수십 개로 넘기 때문에 컨테이너를 넣는 단계에서 함께 고친다. 캔버스 파일은 컨테이너를
  만나면 저장이 실패하고, `YamlWriter::BeginSequence(nullptr)` 는 아무것도 적지 않은 채
  돌아와 뒤따르는 `EndSequence` 가 부모 블록을 닫는다. 기존 엔진 `.jcanvas` 는 `Array<Vector2>`
  를 대시만 있는 줄 아래에 원소를 적는 모양으로 쓰고, 우리 파서는 그것을 이미 읽는다.
  순서: 값 걸음을 Core 로(동작 불변) → 배열·표 걸음과 쓰기 수정, 스냅샷의 고정 버퍼 제거 →
  커맨드와 인스펙터 → 이 기록을 완료로 고친다.
...
- **D-87. 스크립트 편집기는 Code-OSS 포크 "JBro Script Editor" 이고 `.jscript` 만 다룬다.**
  ...
  **JBro 기능은 전부 내장 확장으로 만든다.** 코어 패치는 하되 제품 모양(기본 UI·배치·메뉴)에만
  쓰고 패치 하나에 바꾸는 것 하나를 이유와 함께 둔다. 무엇을 바꿀지는 미정이며 포크 빌드 전에 정한다.
  **확장 마켓플레이스는 연결하지 않는다.** 포크는 약관상 MS 마켓에 접속할 수 없고, 필요한 확장은
  편집기가 싣는다. `product.json` 한 곳이라 되돌릴 수 있다.
