...
- **D-102. 편집기 코어 패치는 셋이다 - 첫 실행 한국어, 채팅·AI 제거, `.vsix` 설치 차단.** (2026-09-16)
  (`Updates` D-87)
  ...
  **셋을 고른 기준은 "설정이나 확장으로는 닿지 않는가" 다.** 닿는 것은 패치하지 않는다.
  시작 화면(환영 탭)을 뜨지 않게 하는 일은 확장이 `workbench.startupEditor` 기본값을 기여하면
  되므로 목록에서 뺐다. 확장 마켓플레이스는 Code-OSS 가 이미 `extensionsGallery` 키를 갖고
  ...
  **남은 세부 둘**(0002 에서 `inlineCompletions` 를 뺄지, 계정 메뉴를 기본에서 감출지)과
  메뉴 구조는 포크 빌드 전에 정한다(ide-plan §7).
...
- **D-102. 설치본은 `Launcher/` 와 `Editor/<버전 폴더>/` 이고, 런처가 알아서 찾는다.** (2026-09-16)
  (`Updates` D-100)
