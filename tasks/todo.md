...
- `[열림]` **네트워크의 남은 것**(D-122, [network-plan.md](./network-plan.md)). 여섯 단계(뼈대 → WS → Reliable UDP →
  ...
  브라우저 접착(§3-6)의 검증이다.
- `[진행]` **오디오**(D-197·D-198·D-201·D-202, [audio-plan.md](./audio-plan.md)). 여섯 단계(믹서·출력·에셋·2D/3D 소스·스크립트
  서비스·에디터·버스 이펙트)가 모두 섰다. 남은 것은 계획서 §5 의 `[열림]`이다: 버스 중첩·솔로·센드, 디스크 스트리밍, 보이스 단위
  이펙트, Web autoplay unlock·장치 선택·핫 언플러그·포커스를 잃었을 때의 정책, 스펙트럼 시각화·버스 미터.
