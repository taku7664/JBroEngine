...
- 필드를 가진 **구조체 원소의 목록**을 그리지도 옮기지도 못한다(코덱이 없다, D-86).
- bool·int·enum 원소 목록과 목록 끌어 옮기기는 **마우스로 재지 않았다**(D-86).
- 스냅샷이 깊이 4 를 넘는 필드를 조용히 건너뛴다. 지금 닿는 컴포넌트는 없다(D-86).
- `TableOpsOf` 의 `new`/`delete` 를 §14 의 예외로 둘지 확인이 필요하다(D-86).
...
- **D-86. 컨테이너 편집은 컨테이너 전체를 글자로 떠서 되돌린다.** D-82 의 "원소 번호를
  ...
  **남은 것**: bool·int·enum 원소의 글자 옮기기와 목록 끌어 옮기기는 단위 테스트만 있고 마우스로
  ...
  (지금 닿는 컴포넌트는 없다).
  **사용자 확인이 필요한 자리**: `TableOpsOf` 의 키·값 만들기가 `new`/`delete` 를 직접 쓴다.
  소유를 `void*` 로 넘겨야 하는데 `OwnerPtr` 는 소유를 놓는 길이 없어서다. §14 의 MUST 와
  어긋나므로, 예외로 둘지 할당 방식을 바꿀지 정해야 한다.
