...
패널과 툴 창(`Tool/SpriteViewerWindow` 등), 앞으로 붙을 외부 에디터가 **각자 만들지 않고 같은 API 를 부르게** 하는 네 가지다.
...
- `[진행 예정]` **4. 로딩을 공용 API 로.** 툴마다 로딩 연출을 따로 만들지 않고, 진행 중인 일을 등록하면 같은 모양(스피너·문구·진행률)으로
  그리는 공용 API 를 둔다. 툴마다 다른 로딩 경험을 줄이는 것이 목적이다. 지금은 `Widget::LoadingSpinner` 그리기 함수만 있고, 누가 언제
  무엇을 기다리는지는 각자 정한다. 기존 엔진은 `RootDockWindow.cpp` 에서 태스크 목록 앞에 스피너를 그렸다(완료는 초록, 미완료는 빨강) -
  구현 전에 그 화면을 먼저 읽는다. 정할 것: 창 전체를 막는 로딩과 패널 안의 로딩을 나눌지, 진행률을 모르는 일의 표시, 끝나면 1 번 알림으로
  이어 줄지, 워커에서 도는 일의 진행 값을 메인 스레드로 넘기는 방식(`SafePtr` 는 메인 스레드 전용).
