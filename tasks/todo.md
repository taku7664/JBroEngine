...
- `[진행 예정]` **4. 로딩은 비동기 - 로딩 관리자가 스레드를 들고 로드 태스크를 등록받는다**(D-208). 툴마다 로딩 연출을 따로 만들지 않는다.
  ...
  - **실패 알림**(제안, 확인 필요): 실패와 취소는 1 번 알림으로 우측 하단에 띄우고 같은 내용을 로그에도 남긴다. 성공은 기본으로 띄우지 않고,
    태스크가 원할 때만 띄운다 - 캔버스를 열 때마다 "완료" 가 쌓이면 알림이 소음이 된다. 알림을 누르면 현황표나 로그의 그 줄로 간다.
  - **순서.** 한 묶음(아래 로드 컨텍스트) 안의 태스크를 **순서대로**(앞 태스크가 끝나야 다음) 돌릴지 **병렬로** 돌릴지 등록할 때 고를 수 있게 한다.
  ...
  - **로드 컨텍스트와 현황표.** 패널·툴이 태스크 묶음을 로드 컨텍스트로 등록하고, 관리자는 등록된 컨텍스트를 모두 안다.
  ...
  - 정할 것: 관리자와 컨텍스트의 이름, 태스크 이름을 로컬라이징 키로 받을지, 취소(10 번)를 하위 작업 사이에서 확인하는 방식, 현황표를 별도 패널로
    둘지 상태 표시줄(13 번)에서 펼칠지, 순서 모드가 컨텍스트 단위인지 태스크 사이 의존까지 필요한지.
...
- **D-208. 에디터의 로딩은 비동기다 - 로딩 관리자가 워커 스레드를 소유하고, 로드 태스크를 등록받아 현황표로 보인다.** (2026-09-26)
  ...
  고른다(기존 엔진 `Core/Task` 에는 이 설정이 없었다). 현황표는 관리자가 아는 로드 컨텍스트를 공용 그리기 하나로 보인다.
