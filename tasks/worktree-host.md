...
- Windows는 원본과 같은 디렉터리의 일회성 섭도 DLL을 로드한다. 운영체제가 기존 코드를 실행하는
  동안에 원본 출력을 삭제·재배치하고, Reload 후에도 섭도 복사본 하나만 남으며 Unload 후에는
  0개가 되는 것을 실제 DLL 테스트로 확인했다.
- 테스트 전용 V1·V2 DLL을 각각 빌드하고, V1을 실행하는 동안 원본 경로를 V2로 교체한 뒤 Reload해
  내보낸 revision 값이 1→2로 바뀌는 것을 확인했다. 서로 다른 새 바이너리 실행은 증명됐다.
- H5의 리플렉션 기반 `Ref` 필드 순회와 H6의 `ScriptSystem::OnUpdate` 실행 후 로그 변화 실측은 아직
  남는다. 파일 잠금·바이너리 교체 증명만으로 H6 전체를 완료 처리하지 않는다.
