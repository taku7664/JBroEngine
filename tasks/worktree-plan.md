...
- 그 둘을 끝내고 **공유 헤더의 선언을 미리 확정**한 뒤에 나눈다.
- 나누면 워크트리 5 개가 나오지만, 1 인 작업이면 **2~3 개가 실용적**이다.

## 0. 전제 — 지금은 나눌 수 없다

```
git log --oneline    →  641e654 Initial commit
git ls-files | wc -l →  3
git status --short   →  ?? CLAUDE.md  ?? docs/  ?? source/  ?? tasks/
```

**신규 리포는 코드가 하나도 커밋돼 있지 않다.** 워크트리는 커밋된 베이스를 공유하는 구조라
현재 상태로는 만들 수 없다. 분기 전에 커밋이 선행되어야 한다.
...
- [ ] **Stage B 완료** — 오브젝트-컴포넌트 모델 전환, 빌드·테스트 통과
- [ ] **Stage C 완료** — 네임스페이스·이름 적용, 빌드·테스트 통과
- [ ] **공유 헤더 선언 확정** (아래 2.2)
- [ ] Debug / Release x64 빌드 통과 + `JBroTests` 통과
- [ ] **커밋** — 현재 전부 미추적이라 이게 없으면 워크트리 자체가 불가능
...
```
현재 ──▶ [커밋] ──▶ Stage B ──▶ Stage C ──▶ 공유 헤더 선언 확정 ──▶ 골격 커밋
                     (직렬, 나눌 수 없음)                            │
                                                                     ▼
                                        ┌──────────┬──────────┬──────────┬──────────┐
